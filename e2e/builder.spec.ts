...
async function addStep(page: Page) {
  ...
  await page.waitForFunction(
    () =>
      (document.getElementById("steps-el")
...
test.describe("Sequence Builder", () => {
  ...
  test("renders the header and the empty-state hint", async ({
    page,
  }) => {
    await expect(
  ...
  test("Add Step inserts an empty step card with the command picker", async ({
    page,
  }) => {
    await addStep(page)
    // Empty step's trigger button shows the placeholder label.
    await expect(
      page.getByText("— pick a command —"),
    ).toBeVisible()
  ...
  test("command picker filters by name and selects on click", async ({
    page,
  }) => {
    await addStep(page)
    ...
    await expect(
      page.getByRole("button", { name: /^Copy Files\s/ }),
    ).toBeVisible()

    // Click the copyFiles entry → picker closes, step shows the command name.
    await page
      .getByRole("button", { name: /^Copy Files\s/ })
      .click()
  ...
  test("View YAML icon opens the modal showing the current sequence", async ({
    page,
  }) => {
    ...
    await expect(modal.locator("#yaml-out")).toContainText(
      "command: copyFiles",
    )
  })

  test("path typeahead pulls from /queries/listDirectoryEntries and completes on click", async ({
    page,
  }) => {
    ...
    const basePathInput = page
      .locator("[data-path-var]")
      .first()
      .locator('input[placeholder*="/mnt/media"]')

    await basePathInput.fill("/")
    await expect(
      page.locator("#path-picker-popover"),
    ).toBeVisible()
    await expect(
      page.getByRole("button", { name: /^📁 mnt$/ }),
    ).toBeVisible()

    // Click "mnt" → drill into mnt/, picker reopens for /mnt.
    await page
      .getByRole("button", { name: /^📁 mnt$/ })
      .click()
    await expect(basePathInput).toHaveValue("/mnt/")
    await expect(
      page.getByRole("button", { name: /^📁 media$/ }),
    ).toBeVisible()
  ...
  test("Add Path creates a new path-variable card whose value lands in the YAML", async ({
    page,
  }) => {
    await openControlsMenu(page)
    await page
    ...
    const newPathValueInput = pathVarCards
      .nth(1)
      .locator("input")
      .last()
    ...
    await page
      .getByRole("button", { name: "View YAML" })
      .click()
    await expect(page.locator("#yaml-out")).toContainText(
      "/data/anime",
    )
  })

  test("step reorder via the up arrow swaps adjacent steps", async ({
    page,
  }) => {
    ...
    await page
      ...
      .fill("moveFiles")
    await page
      .getByRole("button", { name: /^Move Files\s/ })
      .click()
  ...
  test("step delete via ✕ removes the card after the leave animation", async ({
    page,
  }) => {
    await addStep(page)
    const stepCards = page.locator('[id^="step-"]')
  ...
  test("(+) Add divider inserts an empty step at the chosen position", async ({
    page,
  }) => {
    ...
    await expect(stepCards.nth(2)).toContainText(
      "Move Files",
    )
  })

  test("typing a path into a step field auto-promotes it to a path variable on commit", async ({
    page,
  }) => {
    ...
    const destinationPathInput = page
      .locator(
        'input[data-field="destinationPath"].manual-input',
      )
      .first()
    ...
    await expect(
      pathVarCards
        ...
        .locator('input[type="text"]')
        .last(),
    ).toHaveValue("/data/output")
  })

  test("the URL captures sequence state and is restored on reload", async ({
    page,
  }) => {
    await addStep(page)
    await page.getByText("— pick a command —").click()
  ...
  test("Run via API posts the YAML to /sequences/run, opens the modal, and reflects the umbrella job's status", async ({
    page,
  }) => {
    ...
    await page.route("**/sequences/run", async (route) => {
      ...
      await route.fulfill({
        ...
        contentType: "application/json",
        body: JSON.stringify({
          jobId: "test-umbrella-id",
          logsUrl: "/jobs/test-umbrella-id/logs",
        }),
      })
    })
    ...
    await expect(modal).toBeVisible()
    await expect(
      page.locator("#api-run-jobid"),
    ).toContainText("test-umbrella-id")

    // Logs land in the panel as the SSE stream replays.
    await expect(
      page.locator("#api-run-logs"),
    ).toContainText("Step copy: starting.")
    await expect(
      page.locator("#api-run-logs"),
    ).toContainText("Step copy: completed.")

    // Status badge flips to completed once the done event arrives.
    await expect(
      page.locator("#api-run-status"),
    ).toHaveText("completed")
  })

  test("auto-linked step-output renders as { linkedTo, output: folder } in the YAML and survives reload", async ({
    page,
  }) => {
    ...
    const reloadedYaml = await page
      .locator("#yaml-modal #yaml-out")
      .innerText()
...
test.describe("Sequence Builder — groups", () => {
  ...
  test("a divider's + Parallel button inserts a group card with one inner step + parallel badge", async ({
    page,
  }) => {
    ...
    await addStep(page)
    await page
      .getByRole("button", { name: "➕ Parallel" })
      .first()
      .click()
    ...
    await expect(
      group.getByText("— pick a command —"),
    ).toBeVisible()
    await expect(group.locator(".step-card")).toHaveCount(1)
  })

  test("adding a step to a collapsed group expands it so the new card is visible", async ({
    page,
  }) => {
    ...
    await expect(
      groupCard.locator(".step-card"),
    ).toHaveCount(0)

    // Click "+ Step" inside the group header — group should auto-expand.
    await groupCard
      .locator(
        'button[title="Add a step inside this group"]',
      )
      .click()
    await expect(
      groupCard.locator(".step-card"),
  ...
  test("parallel group lays inner steps side-by-side at desktop width and stacks at narrow width", async ({
    page,
  }) => {
    ...
    const desktopFirst = await innerSteps
      .nth(0)
      .boundingBox()
    const desktopSecond = await innerSteps
      .nth(1)
      .boundingBox()
    ...
    if (desktopFirst && desktopSecond) {
      ...
      expect(
        Math.abs(desktopFirst.y - desktopSecond.y),
      ).toBeLessThan(20)
      expect(desktopSecond.x).toBeGreaterThan(
    ...
    await page.waitForTimeout(50)
    const narrowFirst = await innerSteps
      .nth(0)
      .boundingBox()
    const narrowSecond = await innerSteps
      .nth(1)
      .boundingBox()
    expect(narrowFirst).toBeTruthy()
    expect(narrowSecond).toBeTruthy()
    if (narrowFirst && narrowSecond) {
      expect(narrowSecond.y).toBeGreaterThan(
        narrowFirst.y + narrowFirst.height / 2,
      )
    }
  })

  test("a group can be moved up/down past adjacent top-level items via its header arrows", async ({
    page,
  }) => {
    ...
    await groupCard
      .locator('button[title="Move group down"]')
      .click()
  ...
  test("a top-level step jumps past a group when the down arrow hits it", async ({
    page,
  }) => {
  ...
  test("the divider's + Group / + Parallel buttons insert a group between items", async ({
    page,
  }) => {
    ...
    await page
      ...
      .fill("makeDirectory")
    await page
      .getByRole("button", { name: /^Make Directory\s/ })
      .click()
  ...
  test("step collapse chevron round-trips through the URL", async ({
    page,
  }) => {
    await addStep(page)
    await page.getByText("— pick a command —").click()
    ...
    await stepCard
      .locator('button[title*="Collapse step"]')
      .click()
  ...
  test("recursiveDepth is saved even if the field was not blurred before reload", async ({
    page,
  }) => {
    await addStep(page)
    await page.getByText("— pick a command —").click()
    await page
      .getByPlaceholder("Search commands…")
      .fill("deleteFilesByExtension")
    await page
      .getByRole("button", {
        name: /^Delete Files by Extension\s/,
      })
      .click()
    ...
    await expect(
      page.getByRole("spinbutton", { name: /Depth/ }),
    ).toHaveValue("5")
  })

  test("DSL rules Predicates panel stays open after adding a predicate entry", async ({
    page,
  }) => {
    await addStep(page)
    await page.getByText("— pick a command —").click()
  ...
  test("DSL rules When panel stays open after adding a when clause", async ({
    page,
  }) => {
    await addStep(page)
    await page.getByText("— pick a command —").click()
  ...
  test("folderMultiSelect clears selected folders when source path changes", async ({
    page,
  }) => {
    await page.route(
      "**/queries/listDirectoryEntries",
    ...
    await page
      .getByPlaceholder("Search commands…")
      .fill("storeAspectRatioData")
    await page
      .getByRole("button", {
        name: /^Store Aspect Ratio Data\s/,
      })
      .click()
    ...
    await page
      ...
      .click()
    await expect(
      page.locator("#folder-picker-modal"),
    ).toBeVisible()
    await page
    ...
    const folderTag = stepCard.locator("span.font-mono", {
      hasText: "Anime",
    })
  ...
  test("typing in DSL scaleResolution width field does not lose focus", async ({
    page,
  }) => {
    await addStep(page)
    await page.getByText("— pick a command —").click()
...
test.describe("Sequence Builder — step card status badge", () => {
  ...
  test("status badge updates to 'completed' after a successful Run via API", async ({
    page,
  }) => {
    ...
    await page.route("**/sequences/run*", async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          jobId: "umb-badge-ok",
          logsUrl: "/jobs/umb-badge-ok/logs",
        }),
      })
    })
    ...
    await expect(
      stepCard.locator(".status-badge"),
    ).toHaveText("completed")
  })

  test("status badge updates to 'failed' after a failed Run via API", async ({
    page,
  }) => {
    await addStep(page)
    await page.getByText("— pick a command —").click()
    ...
    await page.route("**/sequences/run*", async (route) => {
      await route.fulfill({
        ...
        contentType: "application/json",
        body: JSON.stringify({
          jobId: "umb-badge-fail",
          logsUrl: "/jobs/umb-badge-fail/logs",
        }),
      })
    })

    await page.route(
      "**/jobs/umb-badge-fail/logs",
      async (route) => {
        const sseBody = [
  ...
  test("step card clears stale 'failed' state when re-run succeeds", async ({
    page,
  }) => {
    await addStep(page)
    await page.getByText("— pick a command —").click()
    ...
    await page.route("**/sequences/run*", async (route) => {
      ...
      await route.fulfill({
        ...
        contentType: "application/json",
        body: JSON.stringify({
          jobId: `umb-rerun-${suffix}`,
          logsUrl: `/jobs/umb-rerun-${suffix}/logs`,
        }),
      })
    })

    await page.route(
      "**/jobs/umb-rerun-f/logs",
      async (route) => {
        const sseBody = [
