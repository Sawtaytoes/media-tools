...
**Parallel with:** 29 (openapi-codegen-optional), 2a (server-template-storage), 2c (pure-functions-sweep), 2d (asset-fallback-to-cli), 3b, 3c, 3e, 40 — none of these touch the central logging surface. NOT parallel with 38 (per-file-pipelining), 2b (error-persistence-webhook), or 2f (ffmpeg-gpu-reencode) — those depend on this one landing first.
