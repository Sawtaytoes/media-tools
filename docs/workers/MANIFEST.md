...
| 2c | [pure-functions-sweep](2c_pure-functions-sweep.md) — extract pure cores from `packages/server/src/tools/**`; thin wrappers retain exported signatures; refactor-only, no behavior changes; excludes `nameSpecialFeatures*` (Phase 3) and `app-commands/**` (worker 38) | srv+web | Sonnet | High | ON | 20 | in-progress |
