...
| 58 | [promptmodal-cancel-and-play-fix](58_promptmodal-cancel-and-play-fix.md) — **asap user-blocking fix.** Two coupled PromptModal bugs surfaced the moment the prompt-event wiring shipped (`useLogStream`/`SequenceRunModal` routing `type: "prompt"` → `promptModalAtom`): (a) the `▶ Play` button silently no-ops because `window.openVideoModal` lives inside `FileExplorerModal` and the modal returns `null` unless the explorer is open; (b) no clear cancel path — backdrop-click and Escape just hide the modal locally without telling the server, leaving the pipeline blocked. Lift the video sub-modal out into a standalone atom-driven `VideoPreviewModal`; add explicit `Cancel job` (red, fires `DELETE /jobs/:id`) + `Close (job stays running)` buttons; add a paused-pipeline header; rewrite Escape to be non-destructive and add `Ctrl+C` as the destructive shortcut. Drops `window.openVideoModal` from `types.window.d.ts`. | web | Sonnet | Medium | ON | — | planned |
