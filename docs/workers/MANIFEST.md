...
| 28 | [threadcount-variable-registry-unification](28_threadcount-variable-registry-unification.md) — cleanup follow-up to worker 11: registers `threadCount` in the unified Variables registry from worker 36 (was a parallel side-channel); makes TypePicker registry-driven. Originally slotted for the Phase 4 structured-logging worker; that worker relocated to id `41` per the "never renumber" rule. | web | Sonnet | Medium | ON | 11, 36, 37 | in-progress |
...
| 54 | [bare-console-log-to-structured-loginfo](54_bare-console-log-to-structured-loginfo.md) — sweep ~57 bare `console.log`/`warn`/`error` calls across ~27 files in `packages/server/src/**` over to worker 41's structured `logInfo`/`logWarning`/`logError`. Surfaced by worker 23's code review. Refactor-only, no behavior change. Scope intentionally limited to `packages/server` + `packages/tools` (frontend has no structured-log bridge, CLI is its own concern). Excludes scripts, the startup banner (runs pre-logger), `logMessage.ts` itself, and tests that deliberately spy on `console.*`. | srv | Sonnet | Medium | ON | 41 | planned |
