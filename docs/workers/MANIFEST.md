...
| 2b | [error-persistence-webhook](2b_error-persistence-webhook.md) — on-disk job-error store + delivery state machine with backoff; boot-time replay of pending webhook deliveries; `/api/errors` routes | srv | Sonnet | Medium | ON | 41 | in-progress |
