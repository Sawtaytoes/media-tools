...
| W7D | 7 — PathField typing behavior + PathPicker wiring | ✅ Done | 2026-05-11 | Sonnet medium. Bug 1: onChange dispatches `setPathValue` when linked to path var. Bug 2: first-type with no existing param creates+links a new path var via `addPathVar`+`setLink`. Bug 3: PathPicker wired to PathField + PathVarCard onChange with 250ms debounce. 4 new tests. `setPathValue`/`addPathVar` added to `useBuilderActions`; `addPathVarAtom` added to `sequenceAtoms`. |
| W7E | 7 — SubtitleRulesField default-rules preview section | ✅ Done | 2026-05-11 | Sonnet medium. Prompt: [react-migration-prompts/W7E.md](react-migration-prompts/W7E.md). Collapsible amber panel (DEFAULT_RULES_PREVIEW: setScriptInfo×2 + setStyleFields), useState(true) default-open, RuleCard isReadOnly=true. 3 new tests (render/hide/collapse). 364/364 tests, typecheck clean. |
| W7F | 7 — DnD drop indicator + group droppable zone + DnD tests/stories | ✅ Done | 2026-05-11 | Sonnet medium. Prompt: [react-migration-prompts/W7F.md](react-migration-prompts/W7F.md). 6 streams: ring-2 drop highlight on StepCard+GroupCard; useDroppable+min-h-[3rem] on group body; GroupCard isDragOverlay + floating overlay; 6 dragReorderAtom unit tests (all move paths); BuilderSequenceList.stories.tsx 5 variants; GroupCard SingleStep variant. Gate: 373/373 tests, typecheck clean, lint 1 pre-existing. |
...
| W7F | 2026-05-11 | feat(stories): BuilderSequenceList — 5 stories covering empty/sequential/group/parallel/mixed layouts |
| W7F | 2026-05-11 | feat(stories): GroupCard — add SingleStep variant |
| W7F | 2026-05-11 | chore(checklist): W7F ✅ Done — gate: 373/373 tests, typecheck clean, lint 1 pre-existing warning |
| W7E | 2026-05-11 | feat(subtitle-rules): restore default-rules read-only preview panel + 3 tests |
| W7E | 2026-05-11 | chore(checklist): W7E ✅ Done — gate: 364/364 tests, typecheck clean |
| (orchestrator) | 2026-05-11 | Merge W7E (PR #67) + W7F (PR #69) → react-migration; resolved checklist conflicts |
...
| W7D | 2026-05-11 | fix(path-field): typing when unlinked creates and links a new path variable |
| W7D | 2026-05-11 | fix(path-picker): wire directory-listing dropdown to PathField + PathVarCard inputs |
| W7D | 2026-05-11 | chore(checklist): W7D ✅ Done — 1061 tests, typecheck clean, lint 1 pre-existing warning |
| (orchestrator) | 2026-05-11 | Merge W7D (f702d23) → react-migration; 376/376 tests passing |
