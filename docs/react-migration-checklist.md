...
| W6A | 6 — E2E completion (continues W4B) | ✅ Done | 2026-05-11 | Sonnet medium. 36 e2e tests pass / 5 skipped (2 drag-drop deferred to W6B, 3 video-seek need global FileVideoPlayer mount). 1040 unit tests. Fixed SPA fallback routing in server.ts (per-request index.html read), added aria-labels to StepCard controls (aria-label vs title semantics), fixed step-actions always-visible at wide viewport, added data-rule-key/data-details-key/data-predicate-key to DSL components, added aria-label to NumberWithLookupField lookup button. New: e2e/step-controls.spec.ts (8 tests: play/stop disabled/enabled, up/down disabled+reorder, delete count+identity). |
