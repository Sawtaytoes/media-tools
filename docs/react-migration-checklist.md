...
| W7 | 7 — Replace SortableJS with @dnd-kit (React-native DnD) | ⬜ Not Started | — | Sonnet medium. Prompt: [react-migration-prompts/W7.md](react-migration-prompts/W7.md). Runs in **worktree** `.claude/worktrees/w7` on `feat/dnd-kit-migration`. Sequential after W6 (so W6's e2e specs land against current SortableJS DOM before this migrates them). Replaces `sortablejs` + `@types/sortablejs` with `@dnd-kit/core` + `@dnd-kit/sortable` + `@dnd-kit/utilities`. Adds keyboard a11y (arrows + Space + Escape for reorder). Drops `useDragAndDrop` hook in favor of direct composition at consumers (StepCard / GroupCard). |
