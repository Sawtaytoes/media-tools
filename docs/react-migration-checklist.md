...
After the Final PR is merged and the user confirms migration complete, the project will be renamed from `Mux-Magic` → **MuxMagic**. This includes npm scope (`@mux-magic/*` → `@muxmagic/*`), repo/directory rename, Docker image tag, and republishing `@mux-magic/shared` under `@muxmagic/shared`.
...
| W4A | 4 — Verification & Master Merge | ðŸŸ¡ Audit done; master merge reverted | 2026-05-10 | claude-sonnet-4-6 medium. All 4 gates passed (1004 tests, typecheck, lint, build). 36 YAML fixtures: zero diff. Checklist audited — all rows verified. types.window.d.ts: 7 globals annotated (2 implemented, 5 W5 parity-traps). Storybook: 118 stories. **Master merge (22b48b0) + tag `react-migration-complete` were force-rewound by orchestrator at user's request — user wants to manually verify the migration end-to-end before master is updated.** W4A's audit work and capture-script update remain on react-migration; re-merge to master happens after user verification + W4B + W5. |
| W4B | 4 — E2E tests (worktree off post-W3 react-migration) | ðŸŸ¡ Specs authored; ready to rebase | 2026-05-10 | claude-sonnet-4-6 medium. 4 spec files authored (jobs, modals, drag-drop, dsl-rules). `yarn e2e` was blocked by playwright config baseURL mismatch (pointed at API server, but React SPA is on web server post-W3) — orchestrator fixed config (use.baseURL → webBaseURL) and deleted dead legacy e2e/builder.spec.ts (12 refs to pre-React DOM globals). W4B can now rebase onto react-migration HEAD and run their full gate; merge to master deferred until user finishes manual verification. |
...
| W8H | 8 — B12 paste/delete card animations | ✅ Done | 2026-05-11 | Haiku. TDD: 3 failing tests first (removeStep, removeGroup, pasteCardAt startViewTransition). Fix: wrapped all 3 call sites with `document.startViewTransition(() => flushSync(fn))` matching existing B1 move-handler pattern. Updated removal tests to mock transition synchronously. Commits: 2865789 (tests), 052f36a (fix). All 24 StepCard+GroupCard tests pass. |
| W8I | 8 — B14 setStyleFields autocomplete dropdowns | ✅ Done | 2026-05-11 | Brief: [docs/workers/w8i.md](workers/w8i.md). Sonnet ON medium. TDD: failing tests committed first (0262da8), fix in abf29fc. New AssFieldPicker component (local dropdown, no global atom changes); assFields.ts with STYLE_FIELDS + SCRIPT_INFO_FIELDS. StyleFieldRow field-key + ComputeFromEditor property both use picker; scope-aware options; custom values via Enter. 1166 tests pass, typecheck + lint clean. |
| W8J | 8 — B15 path var deletion in-use guard | ✅ Done | 2026-05-11 | Brief: [docs/workers/w8j.md](workers/w8j.md). Sonnet ON medium. Two-phase delete: removePathVarAtom deletes immediately (no refs) or stages pendingPathVarDeleteAtom; PathVarCard inline amber prompt shows per-field replace/unlink choices; confirmPathVarDeleteAtom applies atomically. TDD: failing tests committed first (6177ba4), fix (76b1958). 10/10 new tests + 6 PathVarCard tests pass, typecheck + lint clean. |
| W9A | 9 — Interactive StepCard + GroupCard Storybook stories | ✅ Done | 2026-05-11 | Brief: [docs/workers/w9a.md](workers/w9a.md). Sonnet ON medium. 2 commits: (1) failing test (TDD); (2) feat — 8 StepCard stories + 5 GroupCard stories + 7 tests with real commandsAtom/stepsAtom + dnd-kit DndContext. 1166/1166 tests, typecheck clean, lint clean. |
| W7B | 7 — LinkPicker bug fixes (stored format + alignment + step paths + footer) | ⬜ Not Started | — | Sonnet medium. Prompt: [react-migration-prompts/W7B.md](react-migration-prompts/W7B.md). Runs in **worktree** `.claude/worktrees/w7b` on `fix/link-picker`. **[PARITY]** Four bugs: (1) `selectItem` stores `"path:basePath"` instead of bare `"basePath"` — breaks YAML and selection; (2) dropdown right-aligned, should be center-aligned; (3) step items show no output path detail (hardcoded `detail: ""`); (4) missing "Don't see what you need?" footer hint. Parallel with W7A + W7C. |
| W7C | 7 — Sequence Runner crash (Content-Type mismatch) | ⬜ Not Started | — | Sonnet medium. Prompt: [react-migration-prompts/W7C.md](react-migration-prompts/W7C.md). Runs in **worktree** `.claude/worktrees/w7c` on `fix/sequence-runner`. **[PARITY]** Client sends `Content-Type: application/yaml` raw text but route only declares `application/json` — `body.steps` is undefined → crash. Fix: three fetch calls wrap YAML in `JSON.stringify({ yaml })` + defensive guard in `sequenceRunner.ts`. Parallel with W7A + W7B. |
...
| W8A | 2026-05-11 | chore(checklist): W8A in progress — B1 arrow animations, B2 intra-group drop, B13 fresh refs, B3 SortableJS trial |
...
| W8J | 2026-05-11 | fix(pathsAtom): B15 — two-phase delete with pendingPathVarDeleteAtom; inline amber prompt in PathVarCard; 10/10 tests pass |
| W8J | 2026-05-11 | chore(checklist): W8J ✅ Done |
