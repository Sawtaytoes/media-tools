...
| W7E | 7 — SubtitleRulesField default-rules preview section | ⬜ Not Started | — | Sonnet medium. Prompt: TBD. **[PARITY]** When `hasDefaultRules` is checked, the builder should show a read-only collapsible section ABOVE predicates titled "Default rules (run before user rules; readonly):" in amber color, showing the representative default rules (`setScriptInfo ScriptType=v4.00+`, `setScriptInfo YCbCr Matrix=TV.709`, `setStyleFields` with margin values). React port AND master both lost this section; origin unclear (no git blame without full audit). Parallel with W7A/B/C/D (disjoint files — `SubtitleRulesField.tsx` + `DslRulesBuilder.tsx`). |
| W7F | 7 — DnD drop indicator + group droppable zone | ⬜ Not Started | — | Sonnet medium. Prompt: TBD. **[PARITY]** Three related issues in `BuilderSequenceList` + `GroupCard` + `dragReorderAtom`: (1) no visual drop indicator showing WHERE the dragged item will land — add `onDragOver` to `DndContext`, track `overId`, highlight the "over" item/container with a blue accent; (2) group inner container is not droppable when empty or when hovering blank space — add `useDroppable` to the inner div and handle `overId = groupId` in `dragReorderAtom` as append-to-end; (3) `DragOverlay` in `BuilderSequenceList` only renders `StepCard` — dragging a group only dims it (no floating overlay). Parallel with W7A/B/C/D/E (disjoint files). |
