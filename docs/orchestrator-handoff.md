# Orchestrator Handoff â€” React Migration Recovery

You are the new orchestrator for the Mux-Magic React migration recovery. The prior orchestrator (Claude Opus 4.7) handed off because the session got expensive. **You are recommended to run as Sonnet 4.6, high effort** â€” the architectural work is done; what remains is execution. Switch to medium effort if budget tightens; don't drop to Haiku (orchestration needs nuanced prompt writing).
...
The repo is in the middle of a vanilla-JS â†’ React 19 migration that previously drifted from its own checklist. A recovery plan (see [react-migration-recovery-handout.md](react-migration-recovery-handout.md)) was authored to reach feature parity with the pre-migration vanilla builder.
...
- Same number + letter (W2A, W2B, W5A, W5B, W6A, W6B) â†’ run concurrently within a phase
- New number (W6 â†’ W7) â†’ sequential next phase, not a sibling of W5A/B/C
...
|---|---|---|---|
| Pre-W0 â†’ W5C | 0â€“5 | âœ… All Done + merged | various |
| W6A | 6 â€” e2e completion | ðŸ”„ Running (user just spawned) | `.claude/worktrees/w6` on `e2e-completion` |
| W6B | 6 â€” dnd-kit swap | âœ… Done + merged (commit `dc11213`) | was `.claude/worktrees/w6b` on `feat/dnd-kit-migration` |
| W7 | 7 â€” Modal primitive + Storybook reorg + missing toggle | â¬œ Ready (not spawned) | will run in `.claude/worktrees/w7` on `feat/storybook-reorg` |
...
1. **Maintain the checklist.** When a worker reports done or partially done, update [react-migration-checklist.md](react-migration-checklist.md) â€” both the Worker Status table and the Progress Log.
...
4. **Fix small inline issues** the user reports during verification â€” duplicate checkboxes, missing imports, config gaps, etc. Keep these to ~10 minutes of work each. Anything larger gets its own worker prompt.
...
6. **Be a scope gardener** â€” see the "Scope discipline" section below.
...
## Scope discipline â€” the [PARITY] / [POLISH] split

The user is explicitly limiting scope to **parity with the previous version** for now. They have a backlog of new features (in `g:\Anime\Mux-Magic-tasks.md`, their personal tasks doc â€” not in the repo) that's deferred until parity is reached.

Categories you should flag:

- **`[PARITY]`** â€” restoring something that worked in the previous version. Default ship.
- **`[POLISH]`** â€” improvement beyond parity. The user will decide keep/defer/kill.
...
- Observability that blocks verification (version footer that displays the git SHA â€” currently stuck on "dev" per W5B's pending build-script fix)
...
- The `e2e-tests` branch is W4B's legacy â€” W6A extends it as `e2e-completion`.
...
3. Run `yarn install` if the merge changed `package.json`/`yarn.lock` â€” common after worker branches add deps
4. Run the full gate (`yarn test run && yarn typecheck && yarn lint`)
5. Push

---

## Recent infrastructure fixes â€” keep these in mind
...
## Open verification findings â€” not yet resolved

These are things the user has flagged during verification that aren't yet addressed by a shipped fix:

1. **Version footer shows "git: dev Â· built dev Â· node v24.15.0"** even in Docker â€” `scripts/build-version.cjs` isn't running in production builds. Captured in W5B's scope expansion (Stream 5 of W5B.md) but W5B reported done without fixing this specifically. W7 or a small inline fix needed.

2. **e2e tests never wired into CI** â€” W6A is running now to address this; it's restoration (the legacy e2e suite worked before the React migration, just wasn't ported to current CI).

3. **PathField breadcrumb still shows `//media`** (doubled slash) â€” may have been a symptom of the failed API call before Vite proxy fix landed. If user reports it again, investigate `FileExplorerModal`'s breadcrumb rendering.

4. **PromptModal** untested in W4B/W6A's specs â€” triggered by SSE prompt events; complex to e2e-test without a real running job. Deferred to W5 / future worker.

5. **CommandHelpModal doesn't render in Storybook** â€” captured in W7's prompt as part of the audit of broken modal stories.
...
- Forbidden patterns (Universal Rule #4 â€” no snapshot/VRT tests)
...
- `feedback_no_snapshot_tests.md` â€” never use `toMatchSnapshot`, `toHaveScreenshot`, Percy, Chromatic
- `feedback_rename_strategy.md` â€” ESLint catches declarations not references; use Edit with `replace_all` per file for local renames, grep-then-rename for exported symbols
- `feedback_shrinking_inventory_pattern.md` â€” when migrating away from a bridge, keep the inventory file shrinking commit-by-commit
- `feedback_auto_merge.md` â€” user prefers auto-merging when tests pass
- `feedback_commit_convention.md` â€” commit and push after each logical group; don't ask permission
- `feedback_package_manager.md` â€” yarn, never npm/npx
- `feedback_agents_md_rules.md` â€” four code rules incl. no single-letter names
...
3. Wait for the user to:
   - Report a verification finding â†’ triage per scope-gardener flow above
   - Report a worker reporting done â†’ merge the branch, update checklist, push
   - Ask you to draft a prompt for a new finding â†’ follow the prompt-pattern guide above
...
- The user appreciates **explicit reasoning shown** â€” the "Insight" callouts in messages help them understand why you're making decisions.
- The user **dislikes unnecessary hedging or equivocation** â€” pick a recommendation and own it. If they disagree, they'll override.
- The user is **technically deep** â€” don't over-explain basics, but DO explain non-obvious architecture decisions.
- The user is **scope-conscious** â€” they actively notice when you're padding scope. The [PARITY]/[POLISH] tag is a contract you should keep.
