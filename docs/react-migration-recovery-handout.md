# React Migration Recovery â€” Worker Handout

> **How to use this doc:** Find your **Worker ID** in the Assignment Table below. Read the **Universal Rules** section (it applies to you). Then jump to your worker section. Each worker section is self-contained â€” you do not need to read other workers' sections.
...
- [packages/web/src/jobs/yamlSerializer.ts](packages/web/src/jobs/yamlSerializer.ts) line 16 calls `window.mediaTools?.buildParams` â€” implementation in [packages/web/public/builder/js/sequence-editor.js](packages/web/public/builder/js/sequence-editor.js) (~line 723).
...
- `yamlSerializer.ts` and `loadYaml.ts` are **not duplicates** â€” they do opposite operations. Keep separate.
...
## Universal Rules â€” ALL WORKERS READ THIS
...
- No `for`/`for...of`/`while` loops â€” use `forEach`/`map`/`filter`/`reduce`.
...
- If your section says you edit a file that other workers also edit (notably `RenderFields.tsx` dispatcher switch), use the `Edit` tool with surgical changes â€” never `Write` the whole file.
...
- **After every commit you push:** update the checklist's "Progress Log" sub-section with one line: `W<id> | <date> | <commit short message>`. This is part of the same commit as your code change â€” do not split it.
- **When you FINISH and verify the pre-push gate:** edit the checklist to mark your row as `âœ… Done` with the date and a one-line summary of what shipped. Commit as `chore(checklist): W<your-id> complete`.
- **If you escalate or get blocked:** edit the checklist to mark your row as `âš ï¸ Blocked` with the reason. Then notify the user.

The checklist format is defined under [#checklist-template] below â€” use it verbatim.
...
|---|---|---|---|---|---|---|
| **Pre-W0** | â€” | Orchestrator (Claude in current session) updates AGENTS.md with no-snapshot / no-VRT rules | (orchestrator) | â€” | (immediate, before any worker spawns) | `react-migration` |
...
| **W4A** | 4 | Parity verification + checklist audit + merge react-migration â†’ master | Sonnet | Medium | W3 | `react-migration` â†’ `master` (parallel with W4B) |
| **W4B** | 4 | E2E tests (Playwright) â€” authored in `e2e/` worktree off post-W3 react-migration | Sonnet | Medium | W3 (parallel with W4A) | worktree off `react-migration`, merges to `master` after W4A |
| **W5** | 5 | Parity-trap + code-smell + a11y cleanup (formerly W6) | Sonnet | High | W4A + W4B both done | `master` |

**Parallel groups:**
- **W0a, W0b, W0c** â€” all three start after Pre-W0, run concurrently on `react-migration`. They touch disjoint files.
- **W2A, W2B, W2C, W2D** â€” all four start after W1, run concurrently on `react-migration`. They share `RenderFields.tsx` only (one `case` block each).
- **W4A + W4B** â€” both start after W3. W4A stays on `react-migration` for verification + master merge. W4B uses a worktree off `react-migration` and only writes files under `e2e/`. **Coordination:** W4B must NOT modify `package.json` or `yarn.lock` without coordinating with W4A (Playwright is already installed; if a new dep is needed, ping the orchestrator). After W4A merges to master, W4B rebases their worktree branch onto master and merges when done.

**Naming note:** Phase 4 was originally a single W4 (verification+merge) followed by W5 (e2e). They've been renamed W4A and W4B to match the parallel-pair pattern from Phase 2 (W2Aâ€“W2D). The cleanup worker formerly known as W6 is now W5 (next phase). Commit history may still mention W4/W5/W6 by their old labels.

---

## How You (the Orchestrator) Monitor Progress

You don't need to interrupt workers â€” they keep the checklist honest as part of their workflow (see Universal Rule #8). To check status at any time:

1. **Open [docs/react-migration-checklist.md](docs/react-migration-checklist.md).** The top table shows every worker's status (`â¬œ Not Started`, `ðŸ”„ In Progress`, `âœ… Done`, `âš ï¸ Blocked`).
2. **Scan the "Progress Log"** at the bottom â€” newest entries are last. Each line is one push: `W<id> | <date> | <commit message>`. This tells you exactly what shipped without reading git history.
3. **Look for âš ï¸ Blocked rows** â€” these need your attention. Workers add a "reason" line next to the status so you can act without re-deriving context.
4. **Read the per-worker sub-checklists** â€” each phase has a `[ ] step-name` list. You can see exactly how far through their steps a worker is.

If you ever suspect a worker has fallen out of sync with the checklist, that's an emergency â€” the same failure mode that triggered this whole recovery. Stop new work, do a W4A-style audit on whatever's claimed Done, and reconcile.

---

# PRE-W0 â€” Orchestrator Task (Claude in current session)

**Who:** The session that approves this plan (me â€” Claude). **When:** Immediately after plan approval, before spawning W0a/W0b/W0c. **Why pulled out of W0:** Has no dependency on any other Phase 0 work; landing it first means every downstream worker reads the updated AGENTS.md before writing tests.
...
# WORKER W0a â€” Stale Cleanup + Checklist Scaffold

**Model:** Haiku Â· **Thinking:** ON Â· **Branch:** `react-migration` Â· **Prerequisite:** Pre-W0 done Â· **Parallel with:** W0b, W0c
...
### Step 1 â€” Delete filesystem-orphan worktree dirs
...
### Step 2 â€” Delete stale local branches (only if safe)
...
If either has unpushed commits, **STOP**, mark your row âš ï¸ Blocked in the checklist, and ask the user. Otherwise:
...
### Step 3 â€” Create the checklist scaffold
...
- All worker rows with â¬œ Not Started, then mark W0a as âœ… Done (since by the time you push this commit, your work is finished)
...
When both commits are pushed: W0b and W0c continue (they can already be running in parallel â€” they don't depend on W0a finishing, but they DO depend on Pre-W0 being done so they can read AGENTS.md).

---

# WORKER W0b â€” Audit Existing State

**Model:** Sonnet Â· **Effort:** Low Â· **Branch:** `react-migration` Â· **Prerequisite:** Pre-W0 done Â· **Parallel with:** W0a, W0c
...
### Step 1 â€” Confirm dev environment runs
...
Open `http://localhost:5173/` (or via Hono on :3000 if that's the entry â€” check the dev script in [package.json](package.json)). Verify `/` and `/builder` both load without console errors. Capture any errors verbatim for the checklist.

### Step 2 â€” Inspect existing partial component directories
...
- **empty/stub** â€” placeholder content only, no real implementation
- **partial** â€” some real implementation but not wired into RenderFields dispatcher
- **wired** â€” a working component already used in the app

Do NOT modify any of these directories â€” W2Aâ€“W2D will extend them based on your findings.

### Step 3 â€” Record findings in the checklist
...
1. `docs(checklist): record W0b audit findings â€” partial component dir classification`
...
# WORKER W0c â€” Parity Reference Capture (TIME-CRITICAL)

**Model:** Sonnet Â· **Effort:** Medium Â· **Branch:** `react-migration` Â· **Prerequisite:** Pre-W0 done Â· **Parallel with:** W0a, W0b

## Your Mission

Capture YAML output for every command **while the legacy JS still works**. Once W1 replaces `buildParams`, the legacy behavior is no longer observable â€” this capture window only exists during Phase 0.
...
### Step 2 â€” Capture YAML for every command
...
### Step 3 â€” Mark yourself done

Edit [docs/react-migration-checklist.md](docs/react-migration-checklist.md) â€” mark W0c âœ… Done and list how many command fixtures you captured.
...
The new fixture files don't need to be referenced by tests yet â€” W1's `buildParams.test.ts` and Phase 4 are the consumers.

## Commits (push each immediately)

1. `test: capture parity reference YAML for all commands (Phase 0 baseline)`
2. `docs(checklist): W0c complete â€” <N> command fixtures captured`
...
# WORKER W1 â€” Phase 1: Wave B-0 (Critical Path)

**Model:** Sonnet Â· **Effort:** Medium Â· **Branch:** `react-migration` Â· **Prerequisite:** W0 complete

## Your Mission
You are the critical path. Until you finish, W2Aâ€“W2D cannot start. You port command definitions and `buildParams` from vanilla JS into TypeScript, replace the RenderFields placeholder with a real dispatcher, and ship the shared helpers W2Aâ€“W2D will consume.

## Step-by-Step

### Step 1 â€” Port commands.js to TS
...
- Write a `buildParams.test.ts` with explicit inline assertions covering every branch:
  - simple field â†’ params passthrough
  - `links[fieldName] = pathVarId` â†’ resolved to path value with `@` prefix
  - `links[fieldName] = { linkedTo, output }` â†’ resolved to step-output reference
...
- W2Aâ€“W2D will import this. Do not change its API after they start.

### Step 5 â€” Replace RenderFields placeholder with real dispatcher
...
- W2Aâ€“W2D will replace each `<TodoField type="boolean">` etc. with the real `<BooleanField />` etc.
...
### Step 6 â€” Drop the dynamic JS import in BuilderPage
...
### Step 7 â€” Update yamlSerializer to use TS buildParams
...
### Step 8 â€” Freeze the Phase 2 test fixture
Create `packages/web/src/commands/__fixtures__/commands.ts`:
- Export a stable subset of commands (one per field type, minimum) for W2Aâ€“W2D test files.
- After this commit, do not modify this fixture file â€” W2Aâ€“W2D imports must remain stable.
...
# WORKER W2A â€” Bundle A: Primitives

**Model:** Haiku Â· **Thinking:** ON Â· **Branch:** `react-migration` Â· **Prerequisite:** W1 complete
...
- **Shared touch-point:** [packages/web/src/components/RenderFields/RenderFields.tsx](packages/web/src/components/RenderFields/RenderFields.tsx) dispatcher switch. Use `Edit` tool â€” never `Write` the whole file. Each bundle owns distinct `case` blocks.
- **Before every push:** `git pull --rebase origin react-migration` â€” keeps your single-line dispatcher edit on top of other bundles' edits.
...
3. Create or extend:
   ...
   - `packages/web/src/components/BooleanField/BooleanField.test.tsx` â€” with **explicit inline expected values** (no snapshots, no screenshots)
...
# WORKER W2B â€” Bundle B: Enum / Language
...
## Bundle Rules
See **W2A â†’ Universal Bundle Rules** above. They apply identically.
...
# WORKER W2C â€” Bundle C: Arrays + JSON
...
## Special Note
`JsonField` may require special handling for parse errors / multi-line input. Match legacy behavior exactly â€” don't improve the error messages or validation rules. Parity is the goal of this phase.
...
# WORKER W2D â€” Bundle D: Composite (Heaviest)
...
## Bundle Rules
See **W2A â†’ Universal Bundle Rules** above. They apply identically. Plus:

- You also port [packages/web/public/builder/js/fields/step-output-picker.js](packages/web/public/builder/js/fields/step-output-picker.js) â€” used by `PathField` and `NumberWithLookupField`.
...
2. If after reading it becomes clear this is more than a mechanical port â€” meaning it needs new Jotai atoms beyond the existing ones, new state-shape decisions, or refactoring of shared helpers â€” **STOP** and add to [docs/react-migration-checklist.md](docs/react-migration-checklist.md):
   ```
...
# WORKER W3 â€” Phase 3: Final Cleanup

**Model:** Sonnet Â· **Effort:** Medium Â· **Branch:** `react-migration` Â· **Prerequisite:** W2A + W2B + W2C + W2D all complete
...
### Step 1 â€” Replace public/index.html with a React entry
...
- Replace with a minimal HTML shell that has `<div id="root"></div>` and loads `app.tsx` (mirror the structure of `packages/web/index.html` â€” verify it's the canonical Vite entry).
- The `/` route is already wired in React Router to `JobsPage`. After this edit, `/` is React-only.

### Step 2 â€” Delete the legacy builder bundle
```bash
...
### Step 3 â€” Audit and remove `window.mediaTools` references
```bash
grep -rn "window.mediaTools" packages/web/src
```
After W1 and W2Aâ€“W2D, only test stubs or `.d.ts` type files should remain. Convert or delete each one. Delete `packages/web/src/types.window.d.ts` if it exists.

### Step 4 â€” Collapse createRoot calls
...
### Step 5 â€” Remove orphaned vendor scripts
...
### Step 6 â€” Strip dev-only inline preamble
...
# WORKER W4A â€” Phase 4: Verification & Master Merge

**Model:** Sonnet Â· **Effort:** Medium Â· **Branch:** `react-migration` â†’ `master` Â· **Prerequisite:** W3 complete Â· **Parallel with:** W4B

> **Naming:** this worker was labeled W4 in earlier docs; renamed to W4A so the parallel pair W4A + W4B matches the W2Aâ€“W2D pattern.

## Your Mission
Prove parity against W0c's reference YAML, run the full suite, then merge `react-migration` into `master`.

W4B is working in a separate worktree at the same time. **Coordination:** you and W4B share only one file in practice â€” [docs/react-migration-checklist.md](docs/react-migration-checklist.md). Each of you edits only your own row. Use `Edit` (not `Write`); `git pull --rebase` before push. W4B won't touch any of your source files or merge to master before you do.

## Step-by-Step

### Step 1 â€” Full test suite
```bash
yarn test run && yarn typecheck && yarn lint
```
All must pass.

### Step 2 â€” Storybook smoke
```bash
yarn storybook
```
Walk every new component story (every Wave B field, every modal, every picker). Click the controls, verify the component reacts. **Manual visual check only â€” no VRT.**

### Step 3 â€” Parity matrix
...
4. Compare the new YAML output against the fixture YAML â€” byte-for-byte.
5. Any diff â†’ bug. File an issue, fix or escalate, do not merge to master with parity drift.

### Step 4 â€” Audit and finalize the checklist
This is your single most important job â€” the prior migration failed because the checklist drifted from reality.

Read [docs/react-migration-checklist.md](docs/react-migration-checklist.md) row-by-row:
1. For every worker row marked âœ… Done, verify the corresponding code actually exists and works (random-sample 2-3 components per bundle, run their tests, manually exercise them in the dev server).
2. For every "Done" sub-task checkbox (e.g. `[x] BooleanField`), grep the codebase to confirm the file and its test exist:
   ...
   ```
3. If you find a drift (checklist says Done, code says No), **STOP**. Reopen that worker's row as âš ï¸ Blocked, document the gap, and notify the user. Do not merge to master until reality matches the checklist.
4. Once the entire table is verified accurate: mark all rows âœ… Done with completion dates. Add a final entry to "Progress Log": `W4A | <date> | checklist audited; all rows verified against code`.

### Step 5 â€” Merge to master
Follow the repo's merge convention (squash or merge commit â€” check with the user if unclear). Tag the commit:
```bash
...
## Verification Before You Push
All of the above must pass. The parity matrix is the strictest gate â€” every command's YAML must match its fixture exactly.
...
# WORKER W4B â€” Phase 4: E2E Tests (Parallel with W4A)

**Model:** Sonnet Â· **Effort:** Medium Â· **Branch:** worktree off `react-migration` (post-W3 state) â†’ merges to `master` after W4A Â· **Prerequisite:** W3 complete Â· **Parallel with:** W4A

> **Naming:** this worker was labeled W5 in earlier docs; renamed to W4B so the parallel pair W4A + W4B matches the W2Aâ€“W2D pattern. The next-phase cleanup worker is now W5 (formerly W6).
...
- **You only write files under [e2e/](e2e/).** Do not touch [packages/web/src/](packages/web/src/) â€” that's W4A's verification surface.
- **Do NOT modify [package.json](package.json) or [yarn.lock](yarn.lock).** Playwright is already installed. If you need a new dependency, **STOP** and notify the orchestrator â€” the install + yarn.lock change must be coordinated with W4A to avoid a merge conflict on master.
- **Checklist edits:** only your own W4B row. Use `Edit` tool, `git pull --rebase` before push.
- **You do not merge to master.** Wait for W4A to merge react-migration â†’ master. Then rebase your `e2e-tests` branch onto master and merge when your specs pass.

## Step-by-Step

### Step 1 â€” Author specs in [e2e/](e2e/)

Use the existing Playwright config and directory. Cover the major user flows:

1. **Builder round-trip:** open `/builder` â†’ create a sequence with at least one step of each major command type â†’ copy YAML â†’ reload â†’ load YAML back â†’ assert sequence matches.
2. **Jobs SSE:** open `/` (jobs page) â†’ start a job â†’ observe SSE streaming progress â†’ confirm completion state.
3. **Modals:** LoadModal, YamlModal, CommandHelpModal, ApiRunModal, PromptModal, LookupModal, FileExplorerModal â€” each opens, accepts input, closes cleanly.
4. **Drag-and-drop:** reorder steps in a sequence; move steps between groups; assert order persists in YAML output.

### Step 2 â€” Assertion style

**No `toHaveScreenshot()` calls.** Use semantic assertions: `expect(page.getByRole(...)).toBeVisible()`, `toHaveText("literal")`, `toHaveCount(n)`. See Universal Rule #4 â€” no VRT under any framework.

### Step 3 â€” Mark yourself done in the checklist

Edit [docs/react-migration-checklist.md](docs/react-migration-checklist.md) â€” mark each spec checkbox as you complete it, push after each spec.
...
1. When all specs pass and W4A has merged react-migration â†’ master:
   ```bash
   ...
   ```
2. Open a PR `e2e-tests` â†’ `master`, or merge directly per repo convention.
3. After merge, clean up the worktree:
   ...
   ```
4. Mark W4B âœ… Done in the checklist. Then notify the orchestrator that W5 (cleanup) can spawn whenever the user is ready.
...
|---|---|---|---|---|---|---|
| Pre-W0 | â€” | (orchestrator) | â¬œ Not Started | â€” | â€” | AGENTS.md update; blocks W0a/b/c |
| W0a | 0 â€” Stale cleanup + checklist scaffold | <model> | â¬œ Not Started | â€” | â€” | Parallel with W0b, W0c |
| W0b | 0 â€” Audit existing state | <model> | â¬œ Not Started | â€” | â€” | Parallel with W0a, W0c |
| W0c | 0 â€” Parity reference capture | <model> | â¬œ Not Started | â€” | â€” | Time-critical: must run while legacy JS works |
| W1 | 1 â€” Wave B-0 RenderFields | <model> | â¬œ Not Started | â€” | â€” | Blocks W2Aâ€“W2D |
| W2A | 2 â€” Bundle A (primitives) | <model> | â¬œ Not Started | â€” | â€” | |
| W2B | 2 â€” Bundle B (enum/lang) | <model> | â¬œ Not Started | â€” | â€” | |
| W2C | 2 â€” Bundle C (arrays/json) | <model> | â¬œ Not Started | â€” | â€” | |
| W2D | 2 â€” Bundle D (composite) | <model> | â¬œ Not Started | â€” | â€” | DslRulesBuilder may escalate |
| W3 | 3 â€” Final Cleanup | <model> | â¬œ Not Started | â€” | â€” | Blocks on W2A+W2B+W2C+W2D+W2.5 |
| W4A | 4 â€” Verification & Master Merge | <model> | â¬œ Not Started | â€” | â€” | Parallel with W4B |
| W4B | 4 â€” E2E tests (worktree) | <model> | â¬œ Not Started | â€” | â€” | Parallel with W4A; merges to master after W4A |
| W5 | 5 â€” Parity-trap + code-smell + a11y cleanup | <model> | â¬œ Not Started | â€” | â€” | Runs after W4A + W4B both done. (Was W6 before rename.) |

**Status legend:** â¬œ Not Started Â· ðŸ”„ In Progress Â· âœ… Done Â· âš ï¸ Blocked Â· âŒ Failed

## Per-Worker Detail

### Pre-W0 â€” Orchestrator AGENTS.md Update
- [ ] AGENTS.md updated with no-snapshot and no-VRT rules
- [ ] `yarn lint` passes
- [ ] Commit pushed: `docs(agents): add no-snapshot and no-VRT rules`

### W0a â€” Stale Cleanup + Checklist Scaffold
...
### W0b â€” Audit Existing State
- [ ] Step 1: Dev environment runs cleanly (or anomalies recorded)
- [ ] Step 2: Inspect partial component dirs â€” fill in findings below
- [ ] Step 3: Findings committed to checklist

**Phase 0 audit findings (W0b fills in):**
- `EnumField/`: <empty/stub/partial/wired â€” describe>
- `LanguageCodeField/`: <empty/stub/partial/wired â€” describe>
- `PathField/`: <empty/stub/partial/wired â€” describe>
- `NumberField/`: <empty/stub/partial/wired â€” describe>
...
### W4A â€” Phase 4: Verification & Master Merge
- [ ] Full test suite green
- [ ] Storybook smoke walk
- [ ] Parity matrix â€” every command's YAML matches its W0 fixture byte-for-byte
- [ ] Master merge + tag `react-migration-complete`

### W4B â€” Phase 4: E2E (Parallel with W4A, worktree)
- [ ] Builder flow specs
- [ ] Jobs flow + SSE spec
- [ ] Modal specs
- [ ] Drag-and-drop spec

### W5 â€” Phase 5: Parity-trap + code-smell + a11y cleanup (formerly W6)
- [ ] Stream 1: parity quirks held back during port
- [ ] Stream 2: code-smell sweep (getIsX collisions, let+subscribe â†’ lastValueFrom, one component per file)
...
### Port targets (Phase 1 â€” W1)
- [packages/web/public/builder/js/commands.js](packages/web/public/builder/js/commands.js) â†’ `packages/web/src/commands/commands.ts`
- [packages/web/public/builder/js/sequence-editor.js](packages/web/public/builder/js/sequence-editor.js) (`buildParams`) â†’ `packages/web/src/commands/buildParams.ts`
- [packages/web/public/builder/js/util/field-visibility.js](packages/web/public/builder/js/util/field-visibility.js) â†’ `packages/web/src/commands/fieldVisibility.ts`
- [packages/web/public/builder/js/fields/field-label.js](packages/web/public/builder/js/fields/field-label.js) â†’ `packages/web/src/components/FieldLabel/FieldLabel.tsx`

### Port targets (Phase 2 â€” W2Aâ€“W2D)
- All files in [packages/web/public/builder/js/fields/](packages/web/public/builder/js/fields/) â†’ `packages/web/src/components/<Field>/`
- [packages/web/public/builder/js/fields/step-output-picker.js](packages/web/public/builder/js/fields/step-output-picker.js) â†’ W2D
- [packages/web/public/builder/js/components/dsl-rules-builder.js](packages/web/public/builder/js/components/dsl-rules-builder.js) â†’ W2D

### Edit targets (Phase 1 â€” W1)
- [packages/web/src/jobs/yamlSerializer.ts](packages/web/src/jobs/yamlSerializer.ts) â€” drop `window.mediaTools` branch
- [packages/web/src/components/RenderFields/RenderFields.tsx](packages/web/src/components/RenderFields/RenderFields.tsx) â€” replace placeholder with real dispatcher
- [packages/web/src/pages/BuilderPage/BuilderPage.tsx](packages/web/src/pages/BuilderPage/BuilderPage.tsx) â€” drop dynamic JS import

### Delete targets (Phase 3 â€” W3)
