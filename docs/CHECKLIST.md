...
| W24b | **Media-sync only — skip media-tools.** **Transport flipped from MQTT to webhook (Option C).** Reasoning: zero npm deps, alerts-only scope. **Two URLs, two env vars:** `WEBHOOK_ERRORS_PRESENT_URL` (POST'd on sync failure) + `WEBHOOK_ERRORS_CLEARED_URL` (POST'd when user clicks "Dismiss all errors"). Each URL fires its own HA automation directly — no payload-event branching. Either URL unset disables only that side. Both fire points must be wired or the integration is asymmetric. Worker output includes step-by-step HA-side webhook automation setup. See [docs/options/home-assistant-integration.md §8](options/home-assistant-integration.md). |
