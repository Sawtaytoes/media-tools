...
const builder = (yargs: Argv) => (
  ...
  .positional(
    ...
    {
      demandOption: true,
      describe: "Output folder produced by a previous step (e.g. /work/SUBTITLED). Its contents are copied up one level into its parent.",
      type: "string",
    },
  )
  .option(
    "deleteSourceFolder",
    {
      boolean: true,
      default: false,
      describe: "Delete the source folder after copying. By default the source is preserved (debug-friendly).",
      type: "boolean",
    },
  )
)
...
export const flattenOutputCommand: CommandModule<{}, Args> = {
  ...
  describe: "Flatten a chained operation's output: copy files from sourcePath up one level (overwriting originals). Prevents folder nesting from accumulating across chained steps that each have an outputFolderName. The source folder is preserved by default; pass --deleteSourceFolder to also remove it.",
