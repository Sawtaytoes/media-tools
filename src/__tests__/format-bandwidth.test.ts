...
const formatterSrc = realFs.readFileSync(
  path.resolve(__dirname, '../../public/format-bandwidth.js'),
  'utf8',
) as string
...
type BandwidthFormatGlobal = {
  BandwidthFormat: {
    formatBandwidth: (bytesPerSecond: number | null | undefined) => string
    formatRemaining: (bytesRemaining: number | null | undefined, bytesPerSecond: number | null | undefined) => string
    formatEta: (bytesRemaining: number | null | undefined, bytesPerSecond: number | null | undefined) => string
  }
}
...
describe('formatBandwidth', () => {
  ...
  test('returns "" for undefined', () => {
    expect(formatBandwidth(undefined as unknown as number)).toBe('')
  })
})
...
describe('formatRemaining', () => {
  ...
  test('returns "" for null bytesRemaining', () => {
    expect(formatRemaining(null as unknown as number, 1_000)).toBe('')
  })

  test('returns "" for null bytesPerSecond', () => {
    expect(formatRemaining(100_000, null as unknown as number)).toBe('')
  })
})
