...
beforeEach(async () => {
  ...
  mediaTools = {
    ...
    renderAll: vi.fn(),
    renderAllAnimated: vi.fn((afterRender?: (behavior: string) => void) => {
      if (afterRender) {
        afterRender("instant")
      }
    }),
...
describe("cardToYamlStr", () => {
  test("includes only the path-vars referenced by the step", async () => {
    const { cardToYamlStr } = await import("./card-clipboard.js")
    mediaTools.paths = [
    ...
    const parsed = window.jsyaml.load(yamlStr) as { paths: Record<string, unknown>; steps: unknown[] }
  ...
  test("includes path-vars referenced by any inner step of a group, deduped", async () => {
      ...
      steps: [
        {
          ...
          command: "copyFiles",
          params: {},
          links: { sourcePath: "workDir", destinationPath: "outDir" },
          status: null,
          error: null,
          isCollapsed: false,
        },
    ...
    const parsed = window.jsyaml.load(yamlStr) as { paths: Record<string, unknown>; steps: unknown[] }
    expect(Object.keys(parsed.paths).sort()).toEqual(["outDir", "workDir"])
...
describe("pasteCardAt", () => {
  test("inserts a copied step with a fresh id; existing step ids unchanged", async () => {
    ...
    const existingStepOne = mediaTools.makeStep("makeDirectory")
    const existingStepTwo = mediaTools.makeStep("makeDirectory")
    ...
    expect((mediaTools.steps[0] as StepLike).id).toBe("step1")
    expect((mediaTools.steps[2] as StepLike).id).toBe("step2")
    ...
    expect(mediaTools.scrollStepIntoView).toHaveBeenCalledWith(pasted.id)
  })
  ...
  test("group with internal linkedTo: inner refs rewrite to the new step ids", async () => {
    ...
    const link = consumer.links.sourcePath as { linkedTo: string; output: string }
  ...
  test("path collision: existing path's value is preserved, pasted path is dropped", async () => {
    const { pasteCardAt } = await import("./card-clipboard.js")
    mediaTools.paths = [{ id: "workDir", label: "Work Dir", value: "/existing" }]
    ...
    expect(mediaTools.paths[0]).toEqual({ id: "workDir", label: "Work Dir", value: "/existing" })
  })
  ...
  test("paste into a group at a specific position", async () => {
    const { pasteCardAt } = await import("./card-clipboard.js")
    const innerStepOne = mediaTools.makeStep("makeDirectory")
    innerStepOne.id = "innerOne"
    const innerStepTwo = mediaTools.makeStep("makeDirectory")
    ...
    await pasteCardAt({ parentGroupId: "outer", indexInParent: 1 })
    ...
    expect(targetGroup.steps[1].command).toBe("makeDirectory")
  })
