...
import {
  RULE_TYPES, WHEN_CLAUSE_NAMES, APPLY_IF_CLAUSE_NAMES, COMPARATOR_VERBS,
  COMPUTE_FROM_OPS_BARE, COMPUTE_FROM_OPS_ALL, DEFAULT_RULES_PREVIEW,
} from './constants.js'
...
function renderKeyValueRow({ stepId, ruleIndex, clauseName, slot, entryKey, entryValue, isReadOnly }) {
...
function renderWhenSlot({ stepId, ruleIndex, clauseName, slot, slotValue, predicates, isReadOnly }) {
  ...
  const entryRows = Object.entries(slotBody).map(([entryKey, entryValue]) => (
    renderKeyValueRow({ stepId, ruleIndex, clauseName, slot, entryKey, entryValue, isReadOnly })
  )).join('')
...
function renderWhenClause({ stepId, ruleIndex, clauseName, clauseValue, predicates, isReadOnly }) {
...
function renderWhenBuilder({ stepId, ruleIndex, whenValue, predicates, isReadOnly }) {
  ...
  const clauseRows = WHEN_CLAUSE_NAMES
    ...
    .map((clauseName) => renderWhenClause({ stepId, ruleIndex, clauseName, clauseValue: when[clauseName], predicates, isReadOnly }))
    .join('')
...
function renderApplyIfEntryRow({ stepId, ruleIndex, clauseName, entryKey, entryValue, isReadOnly }) {
  const readOnlyAttribute = isReadOnly ? 'readonly' : ''
...
function renderApplyIfClause({ stepId, ruleIndex, clauseName, clauseValue, isReadOnly }) {
  const clause = isPlainObject(clauseValue) ? clauseValue : {}
  const entryRows = Object.entries(clause).map(([entryKey, entryValue]) => (
    renderApplyIfEntryRow({ stepId, ruleIndex, clauseName, entryKey, entryValue, isReadOnly })
  )).join('')
  const removeClauseButton = isReadOnly
    ? ''
    : `<button type="button"
...
function renderApplyIfBuilder({ stepId, ruleIndex, applyIfValue, isReadOnly }) {
  ...
  const clauseRows = APPLY_IF_CLAUSE_NAMES
    .filter((clauseName) => usedClauses.has(clauseName))
    .map((clauseName) => renderApplyIfClause({ stepId, ruleIndex, clauseName, clauseValue: applyIf[clauseName], isReadOnly }))
    .join('')
  const addClauseDropdown = isReadOnly || availableClauses.length === 0
    ? ''
...
function renderComputeFromOpRow({ stepId, ruleIndex, fieldKey, opIndex, op, isReadOnly, isFirst, isLast }) {
...
function renderComputeFromEditor({ stepId, ruleIndex, fieldKey, computeFrom, isReadOnly }) {
  ...
  const opRows = ops.map((op, opIndex) => renderComputeFromOpRow({
    stepId, ruleIndex, fieldKey, opIndex, op, isReadOnly,
    isFirst: opIndex === 0, isLast: opIndex === ops.length - 1,
  })).join('')
...
function renderStyleFieldRow({ stepId, ruleIndex, fieldKey, fieldValue, isReadOnly }) {
  const fieldKeyJson = JSON.stringify(fieldKey).replace(/"/g, '&quot;')
...
function renderRuleBody({ stepId, ruleIndex, rule, predicates, isReadOnly }) {
  ...
  if (rule.type === 'setStyleFields') {
    ...
    const fieldRows = Object.entries(fields).map(([fieldKey, fieldValue]) => (
      renderStyleFieldRow({ stepId, ruleIndex, fieldKey, fieldValue, isReadOnly })
    )).join('')
...
function renderRuleCard({ stepId, ruleIndex, rule, predicates, isReadOnly, totalRules }) {
  ...
  const removeButton = isReadOnly
    ...
    : `<button type="button"
        ...
        class="text-xs text-slate-500 hover:text-red-400 px-1.5">✕</button>`
  const headerBackground = isReadOnly ? 'bg-slate-900/30' : 'bg-slate-800/60'
  const cardBackground = isReadOnly ? 'bg-slate-900/20 border-slate-700/40' : 'bg-slate-800/40 border-slate-700'
...
function renderPredicatesManager({ stepId, predicates }) {
  ...
  const predicateCards = predicateNames.map((predicateName) => {
    ...
    const entryRows = Object.entries(body).map(([entryKey, entryValue]) => renderEntry({ predicateName, entryKey, entryValue })).join('')
...
export function renderRulesField({ step }) {
  ...
  const defaultRuleCards = isHasDefaultRules
    ? DEFAULT_RULES_PREVIEW.map((rule, defaultIndex) => renderRuleCard({
        stepId, ruleIndex: defaultIndex, rule, predicates, isReadOnly: true,
  ...
  const ruleCards = rules.map((rule, ruleIndex) => {
    const card = renderRuleCard({
      stepId, ruleIndex, rule, predicates, isReadOnly: false, totalRules: rules.length,
    })
    const insertStrip = renderInsertRuleStrip({ stepId, insertIndex: ruleIndex + 1 })
