...
import {
  bridge, findStepOrNull,
  getRules, getPredicates, commitRules, commitPredicates,
  generateFreshKey, makeEmptyRule, updateRuleAt,
} from './state.js'
...
export function addPredicate({ stepId }) {
  ...
  commitPredicates({ stepId, nextPredicates: { ...current, [freshName]: {} } })
}
...
export function renamePredicate({ stepId, oldName, newName }) {
  ...
  const current = getPredicates(stepId)
  if (!Object.prototype.hasOwnProperty.call(current, oldName)) { return }
  if (Object.prototype.hasOwnProperty.call(current, trimmedNewName)) { return }
  const next = {}
...
export function addPredicateEntry({ stepId, predicateName }) {
  ...
  const finalKey = generateFreshKey({ baseName: 'key', usedNames: new Set(Object.keys(body)) })
  commitPredicates({ stepId, nextPredicates: { ...current, [predicateName]: { ...body, [finalKey]: '' } } })
}
...
export function setPredicateEntryKey({ stepId, predicateName, oldKey, newKey }) {
  ...
  const current = getPredicates(stepId)
  const body = isPlainObject(current[predicateName]) ? current[predicateName] : {}
  if (!Object.prototype.hasOwnProperty.call(body, oldKey)) { return }
  if (Object.prototype.hasOwnProperty.call(body, trimmed)) { return }
  const nextBody = {}
...
export function addRule({ stepId, ruleType, insertIndex }) {
  ...
  commitRules({ stepId, nextRules: [...current.slice(0, target), newRule, ...current.slice(target)] })
}

export function removeRule({ stepId, ruleIndex }) {
  const current = getRules(stepId)
  commitRules({ stepId, nextRules: current.filter((_, index) => index !== ruleIndex) })
}
...
export function changeRuleType({ stepId, ruleIndex, ruleType }) {
  if (!RULE_TYPES.includes(ruleType)) { return }
  const current = getRules(stepId)
  commitRules({
    stepId,
    nextRules: updateRuleAt({ rules: current, ruleIndex, updater: () => makeEmptyRule(ruleType) }),
  })
}
...
export function setScaleResolutionDimension({ stepId, ruleIndex, group, dimension, value }) {
  const current = getRules(stepId)
  commitRules({
    stepId,
    nextRules: updateRuleAt({
...
export function setScaleResolutionFlag({ stepId, ruleIndex, flagName, value }) {
  ...
  commitRules({
    stepId,
    nextRules: updateRuleAt({
      rules: current, ruleIndex, updater: (rule) => ({ ...rule, [flagName]: value }),
    }),
  })
}
