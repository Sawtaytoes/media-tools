...
const positionTooltipNear = ({ tooltipElement, anchorElement }) => {
  ...
  const computedTop = (
    isOverflowingBottom
    ? Math.max(8, anchorRectangle.top - tooltipRectangle.height - verticalGap)
    : preferredTop
  )

  const preferredLeft = anchorRectangle.left
  const maxLeft = viewportWidth - tooltipRectangle.width - 8
  const computedLeft = Math.max(8, Math.min(preferredLeft, maxLeft))
...
const showTooltipFor = ({ anchorElement }) => {
  const tooltipKey = anchorElement.getAttribute('data-tooltip-key')
...
const handlePointerOut = (event) => {
  ...
  const stillInsideAnchor = (
    ...
    && nextElement.closest('[data-tooltip-key]') === anchorElement
  )
