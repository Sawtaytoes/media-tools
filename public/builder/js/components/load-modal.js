...
import { getPaths, getStepCounter, setStepCounter, setSteps, setPaths } from '../state.js'

const bridge = () => window.mediaTools

export const isGroupItem = (item) =>
  !!(item && typeof item === 'object' && item.kind === 'group')
...
export function loadStepItem(item, COMMANDS) {
  ...
  commandDefinition.fields.forEach((field) => {
    ...
    if (value !== undefined) {
      if (typeof value === 'string' && value.startsWith('@')) {
        ...
        step.links[field.name] = { linkedTo: value.linkedTo, output: value.output || 'folder' }
      } else {
        step.params[field.name] = value
      }
    }
  ...
  if (Array.isArray(commandDefinition.persistedKeys)) {
    commandDefinition.persistedKeys.forEach((persistedKey) => {
      ...
      if (persistedValue !== undefined) step.params[persistedKey] = persistedValue
    })
  }
  return step
}
...
export function loadYamlFromText(text) {
  ...
  if (pathEntries.length > 0) {
    setPaths(pathEntries.map(([id, data]) => ({
      id,
      label: (data && typeof data === 'object' && typeof data.label === 'string') ? data.label : id,
      value: (data && typeof data === 'object' && typeof data.value === 'string') ? data.value : '',
    })))
  } else {
    setPaths([{ id: 'basePath', label: 'basePath', value: '' }])
  }

  // Load steps (bare + groups).
  const stepsData = Array.isArray(parsed.steps) ? parsed.steps : []
