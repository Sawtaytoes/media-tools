...
const installCommandDescriptions = () => {
  window.commandDescriptions = {
    flattenOutput: {
      summary: "Flatten a chained step's output: copies the folder's contents up one level.",
      fields: {
...
describe("command-help-modal", () => {
  test("opens with the command's summary + per-field descriptions", async () => {
    const { openCommandHelpModal } = await import("./command-help-modal.js")
    openCommandHelpModal({ commandName: "flattenOutput" })

    const modalElement = document.getElementById("command-help-modal")
    const titleElement = document.getElementById("command-help-title")
    const bodyElement = document.getElementById("command-help-body")

    expect(modalElement!.classList.contains("hidden")).toBe(false)
    expect(titleElement!.textContent).toContain("flattenOutput")
    expect(bodyElement!.textContent).toContain("copies the folder's contents up one level")
    expect(bodyElement!.textContent).toContain("Output folder produced by a previous step.")
    expect(bodyElement!.textContent).toContain("delete sourcePath after copying")
  })

  test("closes when called without an event", async () => {
    const { openCommandHelpModal, closeCommandHelpModal } = await import("./command-help-modal.js")
    openCommandHelpModal({ commandName: "flattenOutput" })
  ...
  test("no-ops on an unknown command", async () => {
    const { openCommandHelpModal } = await import("./command-help-modal.js")
    openCommandHelpModal({ commandName: "thisCommandDoesNotExist" })
