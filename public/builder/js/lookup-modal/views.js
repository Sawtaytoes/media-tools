...
function renderSimpleSearchResults(state) {
  ...
  state._activeOptions = state.results.map((_, index) => () => window.lookupSelectSimpleByIndex(index))
...
function renderDvdCompareSearchResults(state) {
  const groups = state.results.reduce((accumulator, result) => {
    if (state.formatFilter !== 'all' && state.formatFilter !== result.variant) {
      return accumulator
    }
  ...
  state._activeOptions = groupArray.map((_, index) => () => window.lookupSelectGroup(index))
...
export function renderLookupVariantView(state) {
  ...
  state._activeOptions = group.variants.map((v) => () => window.lookupSelectVariant(v.id, v.variant))
...
export function renderLookupReleaseView(state) {
  ...
  if (state.loading) {
  ...
  } else if (!state.releases || state.releases.length === 0) {
    ...
    state._activeOptions = state.releases.map((r) => () => window.lookupSelectRelease(r.hash, r.label))
    body = state.releases.map((r, index) => `<button onclick="lookupSelectReleaseByIndex(${index})"
      class="text-left text-sm px-3 py-2 rounded-lg border border-slate-600 text-slate-200 hover:bg-blue-700 hover:border-blue-500 transition-colors flex items-center gap-2">
      ${kbdBadge(index)}
