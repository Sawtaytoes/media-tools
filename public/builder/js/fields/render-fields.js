...
function renderField({ step, field, stepIndex }) {
  ...
  if (field.type === 'subtitleRules') return renderSubtitleRulesField({ step, field, stepIndex })
  if (field.type === 'boolean') return renderBooleanField({ step, field })
  if (field.type === 'path') return renderPathField({ step, field })
  if (field.type === 'number') return renderNumberField({ step, field })
  if (field.type === 'enum') return renderEnumField({ step, field })
  if (field.type === 'numberWithLookup') return renderNumberWithLookupField({ step, field })
  if (field.type === 'languageCode') return renderLanguageCodeField({ step, field })
  if (field.type === 'languageCodes') return renderLanguageCodesField({ step, field })
...
export function renderFields({ step, stepIndex }) {
  ...
  cmd.fields.forEach((field) => {
    if (field.visibleWhen && !isFieldVisible(field.visibleWhen, step.params)) return
    ...
    if (group && !renderedGroups.has(group)) {
      ...
      const groupFieldsHtml = group.fields
        .map((fieldName) => {
          ...
          if (!f) return ''
          if (f.visibleWhen && !isFieldVisible(f.visibleWhen, step.params)) return ''
          return renderField({ step, field: f, stepIndex })
        })
        .filter(Boolean)
        .join('')
      if (groupFieldsHtml) {
        fieldsHtml.push(`<div class="${group.layout}">${groupFieldsHtml}</div>`)
      }
    } else if (!groupedFieldNames.has(field.name)) {
      const fieldHtml = renderField({ step, field, stepIndex })
