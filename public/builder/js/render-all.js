...
export function mountStepCardProgress(stepId) {
  ...
  host.className =
    "px-3 py-2 border-b border-slate-700 bg-slate-800/40"
...
function buildStepsHtmlParts() {
  ...
  const { parts: stepParts } = steps.reduce(
    (accumulator, item, itemIndex) => {
      if (isGroup(item)) {
        accumulator.parts.push(
          renderGroupCard({
            ...
            renderStep: (step, index, context) =>
              renderStepCard({ step, index, context }),
          }),
        )
        ...
        accumulator.parts.push(
          isDrawerModeOn
            ? renderStepCompactCard({
                step: item,
                index: accumulator.flatStepIndex,
              })
            : renderStepCard({
                step: item,
                index: accumulator.flatStepIndex,
      ...
      accumulator.parts.push(
        renderInsertDivider({ index: itemIndex + 1 }),
      )
      return accumulator
    },
    {
      parts: [renderInsertDivider({ index: 0 })],
      flatStepIndex: 0,
    },
  )
...
export function renderAll() {
  ...
  const pathParts = paths.map((pathVar, pathIndex) =>
    window.mediaTools.renderPathVarCard(
      pathVar,
      pathIndex === 0,
    ),
  )
  ...
  lastRenderedStepIds.current = new Set(
    allFlatSteps.map((entry) => entry.step.id),
  )
