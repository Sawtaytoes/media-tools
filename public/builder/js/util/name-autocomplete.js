...
const scoreCandidate = (candidate, normalizedQuery, queryWords) => {
  ...
  const candidateWords = new Set(normalizedCandidate.split(/[\W_]+/).filter(Boolean))
...
export const filterCandidates = ({
  ...
  if (trimmedQuery === '') {
    const untimed = allKnownNames.filter((name) => possibleSet.has(name))
    const other = allKnownNames.filter((name) => !possibleSet.has(name))
    return {
  ...
  const queryWords = normalizedQuery.split(/[\W_]+/).filter(Boolean)
...
const renderDropdownHtml = ({ allKnownNames, possibleNames, query }) => {
  const { untimed, other } = filterCandidates({ allKnownNames, possibleNames, query })
...
export const attachAutocomplete = ({
  ...
  const candidateList = Array.isArray(allKnownNames) ? allKnownNames : []
  const possibleList = Array.isArray(possibleNames) ? possibleNames : []
  ...
  const handleDropdownClick = (event) => {
    const target = event.target.closest('[data-autocomplete-pick]')
    if (!target) {
      return
    }
    event.preventDefault()
    const picked = target.getAttribute('data-autocomplete-pick') ?? ''
  ...
  dropdownElement.addEventListener('mousedown', handleDropdownClick)
  return () => {
    inputElement.removeEventListener('input', handleInput)
    inputElement.removeEventListener('focus', handleFocus)
    inputElement.removeEventListener('blur', handleBlur)
    dropdownElement.removeEventListener('mousedown', handleDropdownClick)
  }
}
