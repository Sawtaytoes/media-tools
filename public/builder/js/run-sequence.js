...
import {
  closePromptModal,
  showPromptModal,
} from "./run-sequence/prompt-modal.js"
import {
  updateStepLogs,
  updateStepUI,
} from "./run-sequence/step-results.js"
...
export function syncDryRunUI() {
  ...
  const failureBtn = document.getElementById(
    "failure-mode-btn",
  )
  const failureTrack = document.getElementById(
    "failure-mode-track",
  )
  const failureThumb = document.getElementById(
    "failure-mode-thumb",
  )
  if (failureBtn)
    failureBtn.classList.toggle("hidden", !active)
  if (failureTrack) {
  ...
  if (failureThumb)
    failureThumb.style.transform = failure
      ? "translateX(1rem)"
      : ""
}
...
async function runOneStep(step) {
  ...
  step.jobId = jobId
  if (outputFolderName)
    step.runtimeOutputFolderName = outputFolderName
  updateStepUI(step)
...
export async function runGroup(groupId) {
  ...
  if (!group) return
  const hasContent = group.steps.some(
    (s) => s.command !== null,
  )
...
function waitForJob(jobId, stepId) {
  ...
  return new Promise((resolve, reject) => {
    const handle = createTolerantEventSource(
      `/jobs/${jobId}/logs`,
      {
        onMessage: (data) => {
          if (data.done) {
            ...
            if (
              ...
              reject(
                `Job ended with status: ${data.status}`,
              )
