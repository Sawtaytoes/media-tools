...
import {
  renderLookupReleaseView,
  renderLookupSearchView,
  renderLookupVariantView,
} from "./lookup-modal/views.js"
...
export function openLookup(lookupType, stepId, fieldName) {
  ...
  renderLookupModal()
  document
    .getElementById("lookup-modal")
    .classList.remove("hidden")
  setTimeout(
    () =>
      document
        .getElementById("lookup-search-input")
        ?.focus(),
    50,
  )
}
...
export function closeLookupModal(event) {
  if (
    event &&
    event.target !== document.getElementById("lookup-modal")
  )
    return
  document
    .getElementById("lookup-modal")
...
function renderLookupModal() {
  ...
  titleEl.textContent =
    LOOKUP_TITLES[lookupState.lookupType] ?? "Lookup"
...
export async function runLookupSearch(searchTerm) {
  ...
  try {
    const endpoint =
      lookupState.lookupType === "mal"
        ? "/queries/searchMal"
        : lookupState.lookupType === "anidb"
          ? "/queries/searchAnidb"
          : lookupState.lookupType === "tvdb"
            ? "/queries/searchTvdb"
            : lookupState.lookupType === "tmdb"
    ...
    if (data.error)
      console.error("[lookup search]", data.error)
  } catch (err) {
    lookupState.results = []
...
export function lookupSelectSimpleByIndex(index) {
  ...
  if (step) {
    ...
    const field = cmd?.fields.find(
      (f) => f.name === lookupState.fieldName,
    )
...
export async function lookupSelectVariant(fid, variant) {
  ...
  try {
    ...
    if (data.error) {
      console.error(
        "[listDvdCompareReleases] error",
        data.error,
      )
    } else if (lookupState.releases.length === 0) {
      console.warn(
        "[listDvdCompareReleases] no releases",
        data.debug,
      )
    }
  } catch (err) {
...
export function lookupSelectRelease(hash, label) {
  ...
  if (step) {
    ...
    if (group) {
      const variantSuffix =
        lookupState.selectedVariant &&
        lookupState.selectedVariant !== "DVD"
          ? ` (${displayDvdCompareVariant(lookupState.selectedVariant)})`
          : ""
      const yearSuffix = group.year
        ? ` (${group.year})`
        : ""
