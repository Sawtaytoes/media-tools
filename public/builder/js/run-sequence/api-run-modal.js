...
export function openApiRunModal({ jobId, status }) {
  ...
  document.getElementById("api-run-jobid").textContent =
    jobId ? `job ${jobId}` : ""
  setApiRunStatus(status)
  document.getElementById("api-run-logs").textContent = ""
  hideApiRunProgress()
  document
    .getElementById("api-run-modal")
...
export function closeApiRunModal(event) {
  if (
    event &&
    event.target !==
      document.getElementById("api-run-modal")
  )
    return
  document
    .getElementById("api-run-modal")
...
export async function cancelApiRun() {
  ...
  try {
    await fetch(`/jobs/${apiRunCurrentJobId}`, {
      method: "DELETE",
    })
  } catch (err) {
...
function tailApiRunLogs(jobId, onDone) {
  ...
  apiRunEventSource = createTolerantEventSource(
    `/jobs/${jobId}/logs`,
    {
      onMessage: (data) => {
...
function openApiRunChildProgressStream(childJobId, stepId) {
  ...
  apiRunChildEventSource = createTolerantEventSource(
    `/jobs/${childJobId}/logs`,
    {
      onMessage: (data) => {
        if (data.type === "progress") {
          apiRunChildSnapshot =
            window.ProgressUtils.mergeProgress(
              apiRunChildSnapshot,
              data,
            )
          const host = document.getElementById(
            "api-run-progress-host",
          )
          window.ProgressUtils.paintProgressBar(
            host,
            apiRunChildSnapshot,
          )
          if (stepId)
...
function closeApiRunChildProgressStream() {
  if (apiRunChildEventSource) {
    apiRunChildEventSource.close()
    apiRunChildEventSource = null
  }
  if (apiRunChildStepId)
    unmountStepCardProgress(apiRunChildStepId)
...
function showApiRunProgress(stepId) {
  ...
  const label = document.getElementById(
    "api-run-progress-step-label",
  )
  Array.from(
    host.querySelectorAll(".progress-row"),
