...
function getLookupLinkData(step) {
  const cmd = step.command
    ? (window.mediaTools.COMMANDS?.[step.command] ?? null)
    : null
  if (!cmd) return null
  const lookupField = cmd.fields?.find(
    (candidate) =>
  ...
  if (lookupField) {
    ...
    if (id !== undefined && id !== null && id !== "") {
      ...
      return {
        url: lookup.buildUrl(id, step.params),
        label: lookup.label,
      }
    }
  }
...
function getLinkedValueForStep(step, fieldName) {
  return typeof window.mediaTools?.getLinkedValue ===
    "function"
...
function renderNameSpecialFeaturesResultsHtml(
  step,
  renamePairs,
  summaryRecord,
) {
  const unrenamed = Array.isArray(
    summaryRecord?.unrenamedFilenames,
  )
    ? summaryRecord.unrenamedFilenames
    : []
  const possibleNamesRaw = Array.isArray(
  ...
  const renameRowsHtml = renamePairs
    .map(
      (item) =>
  ...
  const linkBlock = linkData
    ...
    : ""
  const leftoversBlock =
    unrenamed.length > 0
      ? renderInteractiveRenameBlock({
          filenames: unrenamed,
          linkBlock,
        })
      : ""
...
function renderInteractiveRenameBlock({
  filenames,
  linkBlock,
}) {
...
function handleSmartMatchRenames({
  step,
  successfulRenames,
  summaryRecord,
}) {
  if (
    !Array.isArray(successfulRenames) ||
  ...
  if (Array.isArray(summaryRecord?.unrenamedFilenames)) {
    const renamedSet = new Set(
      successfulRenames.map(
        ({ oldFilename }) => oldFilename,
      ),
    )
    summaryRecord.unrenamedFilenames =
      summaryRecord.unrenamedFilenames.filter(
        (name) => !renamedSet.has(name),
      )
  }
  if (Array.isArray(step.results)) {
    successfulRenames.forEach(
...
function wireNameSpecialFeaturesResults({
  body,
  step,
  summaryRecord,
}) {
  ...
  const possibleNamesRaw = Array.isArray(
    ...
    ? summaryRecord.possibleNames
    : []
  const possibleNameObjects = possibleNamesRaw
    .map((entry) =>
      typeof entry === "string"
  ...
  const openSmartMatchModal = () => {
    const unrenamedFilenames = Array.isArray(
      summaryRecord?.unrenamedFilenames,
    )
      ? summaryRecord.unrenamedFilenames
      : []
    openSpecialsMappingModal({
      ...
      skipDiskCheck:
        isDryRun() ||
        localStorage.getItem("useMocks") === "1",
      sourcePath,
      unrenamedFilenames,
    })
  }

  const smartMatchButton = body.querySelector(
    "[data-specials-smart-match]",
  )
  if (smartMatchButton) {
    smartMatchButton.addEventListener(
      "click",
      openSmartMatchModal,
    )
  }
  ...
  const unrenamedForAutoOpen = Array.isArray(
    summaryRecord?.unrenamedFilenames,
  )
    ? summaryRecord.unrenamedFilenames
    : []
  ...
  if (
    hasSmartMatchDataForAutoOpen &&
    !step._smartMatchAutoOpened
  ) {
  ...
  rows.forEach((row) => {
    ...
    const filename =
      row.getAttribute("data-rename-filename") ?? ""
    if (playButton) {
      if (
        sourcePath &&
        ...
        playButton.addEventListener("click", () => {
          window.openVideoModal(
            joinSourcePath(sourcePath, filename),
          )
        })
      } else {
        playButton.remove()
...
async function _handleInteractiveRenameClick({
  filename,
  inputElement,
  sourcePath,
  statusElement,
  ...
  if (!sourcePath) {
    showRenameStatus(
      statusElement,
      "error",
      "Cannot determine source folder for this step.",
    )
    return
  }
...
function formatStepResults(step) {
  ...
  if (commandName === "getAudioOffsets") {
    return results
      ...
      .map((item) => {
        ...
        const label =
          filename ||
          item?.sourceFilePath?.split(/[\\/]/).pop() ||
          ""
        return label
          ? `${label}: ${item?.offsetInMilliseconds}ms`
          : `${item?.offsetInMilliseconds}ms`
      })
      .join("\n")
  }

  if (commandName === "nameSpecialFeatures") {
    const summaryRecord = results.find(
      (item) =>
        item &&
        typeof item === "object" &&
    ...
    const validRenamePairs = renamePairs.filter(
      (item) =>
        item &&
        typeof item === "object" &&
    ...
    if (validRenamePairs.length > 0 || summaryRecord) {
      return {
        html: renderNameSpecialFeaturesResultsHtml(
          step,
          validRenamePairs,
          summaryRecord,
        ),
        wire: (body) =>
          wireNameSpecialFeaturesResults({
            body,
            step,
            summaryRecord,
          }),
  ...
  const renameField =
    results.length > 0 &&
    renameLikeFields.find(([fromKey, toKey]) =>
      results.every(
        (item) =>
          item &&
          typeof item === "object" &&
  ...
  if (renameField) {
    ...
    return results
      .map((item) => `${item[fromKey]} → ${item[toKey]}`)
      .join("\n")
  }
...
export function updateStepUI(step) {
  ...
  const runStopBtn = card.querySelector(
  ...
  if (runStopBtn) {
    ...
    runStopBtn.title = isRunningStep
      ? "Cancel this step"
      : "Run this step only"
  }
  ...
  const emptyEl = card.querySelector(".step-empty")
  ...
  if (formattedHtml || formattedText) {
    ...
    if (resultsEl) {
      if (formattedHtml) {
        const body =
          resultsEl.querySelector(".results-body")
        if (body) {
          body.innerHTML = formattedHtml
          if (
            formatted &&
            typeof formatted.wire === "function"
          ) {
            formatted.wire(body)
          }
        }
