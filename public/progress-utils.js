...
(function (global) {
  ...
  function paintTopBar(bar, fill, label, snap) {
    var ratio = (typeof snap.ratio === 'number') ? snap.ratio : null
    if (ratio === null) {
    ...
    var parts = []
    if (typeof snap.filesDone === 'number' && typeof snap.filesTotal === 'number') {
      parts.push(snap.filesDone + '/' + snap.filesTotal + ' files')
    }
    if (ratio !== null) {
  ...
  function paintProgressBar(container, snapshot) {
    ...
    var filesContainer = container.querySelector('.progress-files')
    var currentFiles = Array.isArray(snap.currentFiles) ? snap.currentFiles : []
    ...
    filesContainer.replaceChildren.apply(filesContainer, rows)
  }
