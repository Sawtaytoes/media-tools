...
async function submitPromptChoice(
  jobId,
  promptId,
  selectedIndex,
) {
...
export function showPromptModal(jobId, promptData) {
  ...
  if (promptData.filePath) {
    ...
    playBtn.className =
      "text-[10px] bg-emerald-700 hover:bg-emerald-600 text-white px-2 py-0.5 rounded font-medium leading-none"
  ...
  optionsEl.innerHTML = ""
  const sortedOptions = [...promptData.options].sort(
    (a, b) => {
  ...
  document._promptKeyHandler = (e) => {
    ...
    if (!Number.isNaN(num)) {
      const match = promptData.options.find(
        (o) => o.index === num,
      )
      if (match)
        submitPromptChoice(
    ...
    if (e.key === " " || e.key === "Spacebar") {
      const skipOpt = promptData.options.find(
        (o) => o.index === -1,
      )
      if (skipOpt) {
        e.preventDefault()
        submitPromptChoice(jobId, promptData.promptId, -1)
      }
      return
    }
    if (e.key === "Escape" || e.key === "-") {
      const cancelOpt = promptData.options.find(
        (o) => o.index === -2,
      )
      if (cancelOpt) {
        e.preventDefault()
        submitPromptChoice(jobId, promptData.promptId, -2)
        return
      }
      const skipOpt = promptData.options.find(
        (o) => o.index === -1,
