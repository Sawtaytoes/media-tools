...
const modal = () =>
  document.getElementById("file-explorer-modal")
const body = () =>
  document.getElementById("file-explorer-body")
const breadcrumb = () =>
  document.getElementById("file-explorer-breadcrumb")
const selectionCount = () =>
  document.getElementById("file-explorer-selection-count")
const footer = () =>
  document.getElementById("file-explorer-footer")
const deleteBtn = () =>
  document.getElementById("file-explorer-delete-btn")
const modeBadge = () =>
  document.getElementById("file-explorer-mode-badge")
const pickerBadge = () =>
  document.getElementById("file-explorer-picker-badge")
const pickBtn = () =>
  document.getElementById("file-explorer-pick-btn")
...
function renderBreadcrumb() {
  ...
  segments.forEach((seg, idx) => {
    ...
    if (isLast) {
      ...
      link.className =
        "text-blue-300 hover:text-blue-200 underline-offset-2 hover:underline truncate shrink-0"
...
function renderModeBadge() {
  ...
  if (deleteMode === "trash") {
    ...
    el.title =
      "Deletes go to the OS Recycle Bin (DELETE_TO_TRASH=true)"
  } else {
    ...
    el.title =
      deleteModeReason ||
      "Deletes are permanent — no recovery (DELETE_TO_TRASH=false)"
  }
}
...
function renderListing() {
  if (entries.length === 0) {
    body().innerHTML =
      '<p class="text-slate-500 text-sm py-4 text-center">Folder is empty.</p>'
    return
  }
  ...
  const rowsHtml = sortedEntries
    .map((entry) => {
      ...
      const isVideo =
        entry.isFile && isVideoFile(entry.name)
...
function wireRowActions() {
  ...
  if (selectAll) {
    selectAll.addEventListener("change", (event) => {
      if (event.target.checked) {
        entries.forEach((e) => {
          if (e.isFile) selectedNames.add(e.name)
        })
      } else {
        selectedNames.clear()
      }
  ...
  body()
    .querySelectorAll("[data-fe-sort]")
    .forEach((th) => {
      th.addEventListener("click", () =>
        setSort(th.getAttribute("data-fe-sort")),
      )
    })
}
...
function joinPath(dir, child) {
  const trimmed = dir.endsWith(pathSeparator)
    ? dir.slice(0, -1)
    : dir
...
async function loadDeleteMode() {
  try {
    ...
    deleteModeReason =
      "Could not determine delete mode (endpoint unreachable)"
  }
  renderModeBadge()
}
...
async function loadListing() {
  body().innerHTML =
    '<p class="text-slate-500 text-sm py-4 text-center">Loading…</p>'
  try {
    const url = new URL(
      "/files/list",
      window.location.origin,
...
export async function openFileExplorer(path, options = {}) {
  ...
  currentPath = path
  pickerOnSelect =
    typeof options.pickerOnSelect === "function"
      ? options.pickerOnSelect
      : null
  renderBreadcrumb()
  renderPickerUi()
...
const videoNameLabel = () =>
  document.getElementById("video-modal-name")
const videoModalStatus = () =>
  document.getElementById("video-modal-status")
      ...
      while (true) {
        ...
        try {
          ...
          if (
            ...
            sb.buffered.start(0) < player.currentTime - 5
          ) {
            sb.remove(
              sb.buffered.start(0),
            ...
            await Promise.race([
              new Promise((res) =>
                player.addEventListener("timeupdate", res, {
                  once: true,
                }),
              ),
...
const isAudioFormatBrowserSafe = (audioFormat) => {
  ...
  return !browserUnsupportedAudioFormats.has(
...
export async function openVideoModal(absolutePath) {
  ...
  const isTranscodingAudio = playbackUrl.includes(
    "/transcode/audio",
  )
  videoModalStatus().classList.toggle(
    "hidden",
    !isTranscodingAudio,
  )
  ...
  const features = await fetch("/features")
    .then((r) => r.json())
    .catch(() => ({}))
  if (
    isTranscodingAudio &&
    features.experimentalFfmpegTranscoding
  ) {
  ...
  const openExternalBtn = document.getElementById(
    "video-modal-open-external",
  )
  ...
  openExternalBtn.onclick = async () => {
    ...
    setTimeout(() => {
      openExternalBtn.textContent = original
    }, 1500)
  }
}
