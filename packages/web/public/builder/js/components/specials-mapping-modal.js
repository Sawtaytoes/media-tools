...
const inferSuffixFromName = (name) => {
  ...
  if (
    ...
    lower.includes("behindthescenes")
  )
...
const ensureModalMounted = () => {
  ...
  modal.id = MODAL_ID
  modal.className =
    "hidden fixed inset-0 z-50 flex items-center justify-center bg-black/70"
...
const renderCandidateOptions = ({
  ...
  const candidateOptions = rankedCandidates
...
const renderRow = ({ index, suggestion }) => {
  ...
  const isLowConfidence =
    topCandidate.confidence < LOW_CONFIDENCE_THRESHOLD
  ...
  const existingSuffix = extractSuffixFromStem(
    suggestion.filename,
  )
  const initialSuffix =
    existingSuffix !== ""
      ? existingSuffix
      : selectedName
        ? inferSuffixFromName(selectedName)
        : ""
  ...
  try {
    ...
    data.entries.forEach((entry) => {
      if (typeof entry?.name === "string") {
        ...
        const stem =
          dot > 0 ? entry.name.slice(0, dot) : entry.name
...
const setStatusMessage = ({ kind, message }) => {
  const statusElement = getModal()?.querySelector(
    "[data-mapping-status]",
  )
  if (!statusElement) {
    return
...
const handleConfirmClick = async () => {
  ...
  const rows = Array.from(
    modal.querySelectorAll("[data-mapping-row]"),
  )
  const renamePlan = rows
    .map((row) => {
      const indexAttribute =
        row.getAttribute("data-mapping-index") ?? "-1"
      ...
      const suffixElement = row.querySelector(
        "[data-mapping-suffix]",
      )
      ...
      const baseName =
        customValue.length > 0 ? customValue : selectValue
      ...
      const actualFilename =
        stemToFullName?.get(suggestion.filename) ??
        suggestion.filename
      return {
        desiredName,
  ...
  const targetNames = renamePlan.map((p) =>
    String(p.desiredName).toLowerCase(),
  )
  ...
  if (uniqueTargets.size < targetNames.length) {
    setStatusMessage({
      kind: "error",
      message:
        "Two or more files would have the same name. Please adjust the selections.",
    })
    return
  }
  ...
  setStatusMessage({
    kind: "info",
    message: `Renaming 0 / ${renamePlan.length}…`,
  })

  const successfulRenames = await renamePlan.reduce(
    async (previousPromise, plan, planIndex) => {
      const accumulator = await previousPromise
      setStatusMessage({
        kind: "info",
        message: `Renaming ${planIndex} / ${renamePlan.length}…`,
      })
      const oldPath = joinSourcePath({
        folder: sourcePath,
        filename: plan.actualFilename,
      })
      const newFilename = ensureExtension({
        desiredName: plan.desiredName,
        originalFilename: plan.actualFilename,
      })
      const newPath = joinSourcePath({
        folder: sourcePath,
        filename: newFilename,
      })
      const rowStatusElement = plan.row.querySelector(
        "[data-mapping-row-status]",
      )
      try {
        ...
        if (!response.ok || data?.ok !== true) {
          ...
          const isAlreadyExists = errorMsg
            .toLowerCase()
          ...
          if (isAlreadyExists) {
            if (rowStatusElement) {
              ...
              rowStatusElement.className =
                "text-[10px] font-mono mt-1 text-slate-400"
            }
          ...
          if (rowStatusElement) {
            ...
            rowStatusElement.className =
              "text-[10px] font-mono mt-1 text-red-300"
          }
          return accumulator
        }
        if (rowStatusElement) {
          ...
          rowStatusElement.className =
            "text-[10px] font-mono mt-1 text-emerald-300"
        }
        const includeInput = plan.row.querySelector(
          "[data-mapping-include]",
        )
        if (includeInput) {
  ...
  if (
    typeof onRenameApplied === "function" &&
...
export const openSpecialsMappingModal = async ({
  ...
  if (
    !Array.isArray(unrenamedFilenames) ||
  ...
  const existingStems = existingFilenames
    ? new Set(
        Array.from(existingFilenames).map((name) => {
  ...
  const unrenamedFiles = presentFilenames.map(
    (filename) => ({
      filename,
  ...
  const getRowValue = (row) => {
    const customInput = row.querySelector(
      "[data-mapping-input]",
    )
    const selectEl = row.querySelector(
      "[data-mapping-select]",
    )
    const suffixEl = row.querySelector(
      "[data-mapping-suffix]",
    )
  ...
  const validateSelections = () => {
    ...
    rows.forEach((row) => {
      ...
      if (v.length > 0)
        selectedNames[v.toLowerCase()] =
          (selectedNames[v.toLowerCase()] || 0) + 1
    })
    let hasDuplicates = false
    rows.forEach((row) => {
      const v = getRowValue(row)
      const errorEl = row.querySelector(
        "[data-mapping-row-error]",
      )
      const customInput = row.querySelector(
        "[data-mapping-input]",
      )
      ...
      const isDuplicate =
        v.length > 0 && selectedNames[v.toLowerCase()] > 1
      if (isDuplicate) {
        hasDuplicates = true
        if (errorEl) {
        ...
        if (
          customInput &&
          !customInput.classList.contains("hidden")
        )
  ...
  modal
    .querySelectorAll("[data-mapping-select]")
    .forEach((el) =>
      el.addEventListener("change", validateSelections),
    )
  modal
    .querySelectorAll("[data-mapping-suffix]")
    .forEach((el) =>
      el.addEventListener("change", validateSelections),
    )
  modal
    .querySelectorAll("[data-mapping-input]")
    .forEach((el) =>
      el.addEventListener("input", validateSelections),
    )
  ...
  const updateSuffixVisibility = (row) => {
    ...
    const suffixRow = row.querySelector(
      "[data-mapping-suffix-row]",
    )
    if (!suffixRow) return
    const customActive =
      customInput &&
  ...
  modal
    .querySelectorAll("[data-mapping-custom]")
    .forEach((button) => {
  ...
  modal
    .querySelectorAll("[data-mapping-row]")
    .forEach(updateSuffixVisibility)
  ...
  const playButtons = modal.querySelectorAll(
    "[data-mapping-play]",
  )
