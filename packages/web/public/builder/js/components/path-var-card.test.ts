...
describe("renderPathVarCard", () => {
  test("emits delegated data-action attributes", async () => {
    const { renderPathVarCard } = await import(
      "./path-var-card.js"
    )
...
describe("setPathLabel", () => {
  test("mutates the matching path and triggers persistence", async () => {
    const { setPathLabel } = await import(
      "./path-var-card.js"
    )
...
describe("removePath", () => {
  test("drops the path and clears matching step links", async () => {
    ...
    mediaTools.steps = [
      {
        id: "step1",
        links: { sourcePath: "wd", other: "basePath" },
      },
    ]
    const { removePath } = await import(
      "./path-var-card.js"
    )
    removePath("wd")
    expect(
      mediaTools.paths.map((pathItem) => pathItem.id),
    ).toEqual(["basePath"])
    expect(
      mediaTools.steps[0].links.sourcePath,
    ).toBeUndefined()
