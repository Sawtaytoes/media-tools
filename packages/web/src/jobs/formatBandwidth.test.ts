// Tests for the bandwidth / ETA formatters.
//
// Ported from packages/web/public/format-bandwidth.test.ts before W3 deleted
// the legacy IIFE bundle. The legacy test used vi.importActual + Function-eval
// to load the IIFE; this version just imports the ES module directly.
...
describe("formatBandwidth", () => {
  ...
  test('returns "" for undefined', () => {
    expect(formatBandwidth(undefined)).toBe("")
  })
})
...
describe("formatRemaining", () => {
  ...
  test('returns "" for undefined bytesRemaining', () => {
    expect(formatRemaining(undefined, 1_000)).toBe("")
  })

  test('returns "" for undefined bytesPerSecond', () => {
    expect(formatRemaining(100_000, undefined)).toBe("")
  })
})
