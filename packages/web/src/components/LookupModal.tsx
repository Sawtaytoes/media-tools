...
const SearchStage = ({
        ...
        ? (state.results as Array<{ groups?: LookupGroup[] }>)
            .map((group) => ({
              ...
              groups: (group.groups ?? []).filter((grp) =>
                grp.variants?.some(
                  (variant) => variant.variant === state.formatFilter,
                ),
              ),
            }))
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {state.searchError && (
        <p className="text-rose-400 text-xs">{state.searchError}</p>
      )}
            ...
            const handleSelect = () => {
              if (state.lookupType === "dvdcompare") {
                ...
                if (group.variants.length === 1) {
                  ...
                  fetchReleases(group.variants[0].id).then(
...
const ReleaseStage = ({
  ...
  if (state.releasesError) {
    return (
      <p className="text-rose-400 text-xs">{state.releasesError}</p>
    );
  }
