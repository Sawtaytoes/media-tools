...
const FieldEntry = ({ commandName, field }: FieldEntryProps) => {
  const description =
    typeof window.getCommandFieldDescription === "function"
      ? window.getCommandFieldDescription({ commandName, fieldName: field.name })
      : ""
    ...
    <div className="border-b border-slate-800 pb-3 last:border-b-0">
      ...
      {description ? (
        ...
        <p className="text-xs text-slate-500 italic">
          ...
          <code className="text-slate-400 bg-slate-950 px-1 rounded">src/api/schemas.ts</code>.
        </p>
      )}
    </div>
  )
...
export const CommandHelpModal = ({ isOpen, onClose }: CommandHelpModalProps) => {
  ...
  const commandLabelText =
    typeof window.commandLabel === "function" ? window.commandLabel(commandName) : commandName
  ...
  return (
    ...
    >
      <div
        ...
        style={{ width: "min(90vw,700px)", maxHeight: "85vh" }}
          ...
          {commandConfig.fields.length > 0 ? (
            <div className="space-y-3">
              ...
              {commandConfig.fields.map((field) => (
                <FieldEntry key={field.name} commandName={commandName} field={field} />
              ))}
            </div>
          ) : (
            <p className="text-xs text-slate-500 italic">
