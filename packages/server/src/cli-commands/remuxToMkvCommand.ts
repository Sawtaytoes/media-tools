...
const builder = (yargs: Argv) => (
  ...
  .option(
    "extensions",
    {
      array: true,
      demandOption: true,
      describe: "List of file extensions to remux (with or without leading dot), e.g. .ts .m2ts.",
      type: "string",
    },
  )
  .option(
  ...
  .option(
    "isSourceDeletedOnSuccess",
    {
      boolean: true,
      default: false,
      describe: "Delete each source file after its remux completes successfully.",
      nargs: 0,
      type: "boolean",
    },
  )
...
export const remuxToMkvCommand: CommandModule<{}, Args> = {
  ...
  describe: "Pass-through remux of every matching file into an .mkv sibling using mkvmerge.",
  ...
  handler: (argv) => {
    remuxToMkv({
      ...
      isSourceDeletedOnSuccess: argv.isSourceDeletedOnSuccess,
