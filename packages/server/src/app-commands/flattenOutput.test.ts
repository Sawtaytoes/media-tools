...
describe(flattenOutput.name, () => {
  ...
  test("copies every file in sourcePath up one level into the parent directory", async () => {
    await firstValueFrom(
      flattenOutput({ sourcePath: "/work/SUBTITLED" })
      .pipe(toArray()),
    )
    ...
    await expect(stat("/work/unrelated.txt")).resolves.toBeDefined()
  })
  ...
  test("preserves the source folder by default so mid-sequence inspection is possible", async () => {
    ...
    await expect(stat("/work/SUBTITLED")).resolves.toBeDefined()
    await expect(stat("/work/SUBTITLED/episode-01.mkv")).resolves.toBeDefined()
  })

  test("removes the source folder when deleteSourceFolder is true", async () => {
    await firstValueFrom(
      flattenOutput({ deleteSourceFolder: true, sourcePath: "/work/SUBTITLED" })
      .pipe(toArray()),
    )
  ...
  test("overwrites a same-named file in the parent with the source copy", async () => {
    ...
    const merged = vol.readFileSync("/work/episode-01.mkv", "utf8")
