...
describe(deleteFolder.name, () => {
  ...
  test("recursively removes the folder when confirm is true", async () => {
    const result = await lastValueFrom(
      deleteFolder({ confirm: true, folderPath: "/work/TEMP/AUDIO-OFFSETS" }),
    )

    expect(result).toBe("/work/TEMP/AUDIO-OFFSETS")
    await expect(stat("/work/TEMP/AUDIO-OFFSETS")).rejects.toThrow()
    await expect(stat("/work/TEMP/AUDIO-OFFSETS/nested/file.bin")).rejects.toThrow()
  ...
  test("refuses to run and emits a propagated error when confirm is false", async () => (
    captureConsoleMessage("error", async () => {
      ...
      await expect(firstValueFrom(
        deleteFolder({ confirm: false, folderPath: "/work/TEMP/AUDIO-OFFSETS" })
        .pipe(toArray()),
      )).rejects.toThrow(/confirm: true/i)

      // Folder must still be intact — confirm: false is a hard stop.
      await expect(stat("/work/TEMP/AUDIO-OFFSETS")).resolves.toBeDefined()
      await expect(stat("/work/TEMP/AUDIO-OFFSETS/offsets.json")).resolves.toBeDefined()
    })
  ))
  ...
  test("logs the refusal reason when confirm is omitted from the call", async () => (
    captureConsoleMessage("error", async (consoleSpy) => {
      await expect(firstValueFrom(
        ...
        deleteFolder({ folderPath: "/work/TEMP/AUDIO-OFFSETS" })
        .pipe(toArray()),
      )).rejects.toThrow(/confirm: true/i)
