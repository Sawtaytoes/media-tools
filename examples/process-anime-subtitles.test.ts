...
const yamlPath = join(__dirname, "process-anime-subtitles.yaml")
...
describe("examples/process-anime-subtitles.yaml", () => {
  test("parses as YAML and validates the document shape (paths + steps with ids)", async () => {
    ...
    const doc = yaml.load(raw) as {
      ...
      steps: Array<{ id?: string; command: string; params?: Record<string, unknown> }>
    }
  ...
  test("every command name in the example is registered in commandConfigs", async () => {
    const raw = await readFileWithoutMock(yamlPath)
    const doc = yaml.load(raw) as { steps: Array<{ command: string }> }
    const unknowns = doc.steps
      .map((step) => step.command)
      .filter((name) => !Object.prototype.hasOwnProperty.call(commandConfigs, name))
  ...
  test("every step's params resolve cleanly through the live resolver (links, paths, named outputs)", async () => {
    const raw = await readFileWithoutMock(yamlPath)
    const doc = yaml.load(raw) as {
      paths: Record<string, { label?: string; value: string }>
      steps: Array<{ id: string; command: string; params?: Record<string, unknown> }>
    }
    ...
    doc.steps.forEach((step) => {
      const config = commandConfigs[step.command as keyof typeof commandConfigs]
      ...
      expect(errors, `step "${step.id}" produced resolution errors`).toEqual([])
      ...
      stepsById[step.id] = {
        ...
        outputs: (
          ...
          ? config.extractOutputs([{ type: "setScriptInfo", key: "ScriptType", value: "v4.00+" }])
          : null
        ),
...
async function readFileWithoutMock(path: string): Promise<string> {
